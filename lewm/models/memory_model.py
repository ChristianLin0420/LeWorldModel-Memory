...
class MemoryLeWorldModel(LeWorldModel):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        elif memory_impl in _CFHIROV13_MODES:               # fitted normal predictive observer
            if (not isinstance(cf_hiro_state_dim, int)
                    or isinstance(cf_hiro_state_dim, bool)
                    or cf_hiro_state_dim < 1):
                raise ValueError(
                    'CF-HIRO-v13 requires a positive fixed cf_hiro_state_dim')
            self.mem_cfhirov13 = CFHIROv13Memory(
    ...
    def _inject(self, z: torch.Tensor, actions: torch.Tensor = None,
        ...
        if self.memory_impl in _CFHIROV13_MODES:
            if actions is None:
                raise ValueError('CF-HIRO-v13 requires actions with a_t mapping z_t to z_{t+1}')
            if memory_update_mask is not None or gate_override is not None:
                raise ValueError('CF-HIRO-v13 has no visibility mask or learned gate override')
            if action_override is not None or resistance_override is not None:
                raise ValueError('CF-HIRO-v13 has no external action/resistance override')
            result = self.mem_cfhirov13(
    ...
    def horizons(self):
        ...
        if self.memory_impl in _CFHIROV13_MODES:
            method = getattr(self.mem_cfhirov13, 'horizons', None)
