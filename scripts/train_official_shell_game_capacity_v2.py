#!/usr/bin/env python3
"""Train one unchanged frozen carrier on an admitted V2 semantic stage."""
...
def _load_admitted_cache_v2(spec: dict, stage: str, split: str) -> dict:
    selection = require_selected_salience_v2(spec, stage)
    manifest_path = cache_manifest_path_v2(spec, stage)
    admission_file = admission_path_v2(spec, stage)
    if not manifest_path.is_file() or not admission_file.is_file():
        raise FileNotFoundError(f"V2 stage {stage} has no formal admission")
    ...
    if record.get("sha256") != sha256_file(admission_file) \
        ...
        raise RuntimeError(f"V2 stage {stage} did not pass unchanged gates")
    if manifest.get("formal_lock") != lock_receipt_v2(spec) \
            or admission.get("formal_lock") != lock_receipt_v2(spec) \
            or manifest.get("development_selection") != selection \
            or admission.get("development_selection") != selection \
            or manifest.get("threshold_changed_from_v1") is not False \
            or admission.get("threshold_changed_from_v1") is not False:
        raise ValueError("V2 admission was produced under a different amendment")
    arrays, sidecar = load_verified_npz(cache_path_v2(spec, stage, split))
    required = {
        "z", "actions", "initial_slots", "final_slots", "cue_on",
        "cue_off", "swap_pairs", "shuffle_off",
    }
    if set(arrays) != required:
        raise ValueError(f"V2 cache fields differ: {sorted(arrays)}")
    if sidecar.get("schema") != "official_shell_game_cache_v2" \
        ...
        raise ValueError("V2 cache metadata differs from the locked amendment")
    if arrays["z"].shape[-1] != OFFICIAL_EMBED_DIM \
            or arrays["actions"].shape[-1] != OFFICIAL_ACTION_DIM:
        raise ValueError("V2 cache does not use official latent/action dimensions")
    arrays["meta"] = sidecar
...
def main(argv: Iterable[str] | None = None) -> None:
    ...
    if args.arm not in training["arms"] or args.seed not in training["seeds"]:
        raise ValueError("requested V2 carrier cell lies outside the locked grid")

    train = _load_admitted_cache_v2(spec, args.stage, "train")
    validation = _load_admitted_cache_v2(spec, args.stage, "validation")
    output = carrier_directory_v2(spec, args.stage, args.arm, args.seed)
    if output.exists():
        raise FileExistsError(f"refusing to overwrite V2 carrier {output}")
    ...
    if not torch.cuda.is_available():
        raise RuntimeError("formal V2 training requires an allowed CUDA device")
        ...
        if carrier.parameter_count() > 0:
            ...
            for epoch in range(1, epochs + 1):
                ...
                if epoch == 1 or epoch % 10 == 0 or epoch == epochs:
                    print(f"[shell-game-v2-carrier] {args.stage}/{args.arm}/"
    ...
    if host_before != host_after:
        raise RuntimeError("frozen official host changed during V2 training")

    selection = require_selected_salience_v2(spec, args.stage)
    metrics = {
        "schema": "official_shell_game_carrier_metrics_v2",
        "study": spec["study"],
        "stage": args.stage,
        "display_name": stage_contract_v2(args.stage).stage.display_name,
        "capacity": stage_contract_v2(args.stage).stage.capacity,
        "arm": args.arm,
        "seed": args.seed,
        ...
        "admission_sha256": sha256_file(
            admission_path_v2(spec, args.stage)),
        "source_caches": {
            split: {
                "path": str(cache_path_v2(spec, args.stage, split)),
                "sha256": sha256_file(cache_path_v2(spec, args.stage, split)),
            }
    ...
    atomic_text(manifest_path, stable_json({
        "schema": "official_shell_game_carrier_manifest_v2",
        "study": spec["study"],
        "stage": args.stage,
        "arm": args.arm,
        "seed": args.seed,
