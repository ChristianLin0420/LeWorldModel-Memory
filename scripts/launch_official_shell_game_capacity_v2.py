#!/usr/bin/env python3
"""Preview or explicitly execute the development-gated shell-game V2 plan."""
...
def parse_gpu_ids_v2(raw: str) -> tuple[int, ...]:
    ...
    for token in tokens:
        ...
        gpu_id = int(token)
        validate_device_v2(f"cuda:{gpu_id}")
        if gpu_id in result:
...
def build_wave_jobs_v2(
        spec: Mapping, wave: str, gpu_ids: Sequence[int],
        spec_path: Path = DEFAULT_SPEC_V2,
        lock_path: Path = DEFAULT_LOCK_V2,
        ) -> list[JobV2]:
    """Build one immutable V2 wave without touching output directories."""

    if wave not in WAVES:
        raise ValueError(f"unknown shell-game V2 wave {wave!r}")
    if not gpu_ids:
        raise ValueError("at least one allowed GPU is required")
    devices = tuple(validate_device_v2(f"cuda:{int(gpu_id)}")
    ...
    if wave == "development-base":
        destination = base_path_v2(spec, "development")
        jobs.append(JobV2(
            "development-base",
            (PYTHON, "scripts/collect_official_shell_game_base_v2.py",
             "--split", "development", "--device", devices[0], *common),
            _sidecar(destination), devices[0]))
    elif wave == "development-stages":
        for stage in stages:
            destination = stage_path_v2(spec, stage, "development")
            jobs.append(JobV2(
                f"development-stage-{stage}",
                (PYTHON, "scripts/prepare_official_shell_game_stage_v2.py",
        ...
        for index, stage in enumerate(stages):
            ...
            jobs.append(JobV2(
                ...
                (PYTHON,
                 "scripts/cache_official_shell_game_development_v2.py",
                 "--stage", stage, "--device", device, *common),
                development_manifest_path_v2(spec, stage), device))
    elif wave == "formal-base":
        for index, split in enumerate(("train", "validation")):
            device = devices[index % len(devices)]
            destination = base_path_v2(spec, split)
            jobs.append(JobV2(
                f"formal-base-{split}",
                (PYTHON, "scripts/collect_official_shell_game_base_v2.py",
        ...
        for stage in stages:
            for split in ("train", "validation"):
                destination = stage_path_v2(spec, stage, split)
                jobs.append(JobV2(
                    f"formal-stage-{stage}-{split}",
                    (PYTHON,
                     "scripts/prepare_official_shell_game_stage_v2.py",
        ...
        for index, stage in enumerate(stages):
            ...
            jobs.append(JobV2(
                ...
                (PYTHON, "scripts/cache_official_shell_game_capacity_v2.py",
                 "--stage", stage, "--device", device, *common),
                cache_manifest_path_v2(spec, stage), device))
    else:
        index = 0
            ...
            for arm in training["arms"]:
                for seed in training["seeds"]:
                    ...
                    jobs.append(JobV2(
                        ...
                        (PYTHON,
                         "scripts/train_official_shell_game_capacity_v2.py",
                         "--stage", stage, "--arm", arm,
                         "--seed", str(int(seed)),
    ...
    if len(names) != len(set(names)) \
        ...
        raise RuntimeError(f"duplicate V2 job cell in {wave}")
    for job in jobs:
        if job.device is not None:
            validate_device_v2(job.device)
    return jobs
...
def build_plan_v2(
        ...
        spec_path: Path = DEFAULT_SPEC_V2,
        lock_path: Path = DEFAULT_LOCK_V2,
        ) -> list[tuple[str, list[JobV2]]]:
    if wave not in ALL_WAVES:
        raise ValueError(f"unknown shell-game V2 wave {wave!r}")
    selected = WAVES if wave == "all" else (wave,)
    return [(name, build_wave_jobs_v2(
        spec, name, gpu_ids, spec_path, lock_path)) for name in selected]


def preview_lines_v2(
        plan: Sequence[tuple[str, Sequence[JobV2]]]) -> list[str]:
    lines = []
...
def parse_args(argv: Iterable[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help="execute V2; without this flag only print the immutable plan")
...
def _next_startable(queue: Sequence[JobV2], running: Sequence[tuple],
...
def _execute_wave(spec: Mapping, wave: str, jobs: Sequence[JobV2],
                  cpu_jobs: int) -> None:
    ...
    if not pending:
        print(f"[shell-game-v2-launch] wave={wave} already complete", flush=True)
        return
    logs = log_root_v2(spec) / wave
    logs.mkdir(parents=True, exist_ok=True)
    queue = list(pending)
    running: list[tuple[subprocess.Popen, JobV2, object]] = []
    completed = failed = 0
    while queue or running:
        while True:
            ...
            print(f"[shell-game-v2-launch] start {job.name} "
                  f"device={job.device or 'cpu'}", flush=True)
        if not running and queue:
            raise RuntimeError("V2 scheduler cannot start a pending job")
        time.sleep(1)
        active = []
        for process, job, stream in running:
            ...
            if code == 0 and job.done_file.is_file():
                completed += 1
                print(f"[shell-game-v2-launch] done {job.name}", flush=True)
            else:
                failed += 1
                print(f"[shell-game-v2-launch] FAIL {job.name} exit={code} "
                      f"log={logs / (job.name + '.log')}", flush=True)
        running = active
    if failed:
        raise SystemExit(
            f"{failed} V2 jobs failed in {wave}; downstream waves blocked")
    print(f"[shell-game-v2-launch] wave={wave} complete={completed}", flush=True)
...
def main(argv: Iterable[str] | None = None) -> None:
    ...
    gpu_ids = parse_gpu_ids_v2(args.gpus)
    spec_path, lock_path = args.spec.resolve(), args.lock.resolve()
    spec = load_locked_spec_v2(spec_path, lock_path)
    plan = build_plan_v2(spec, args.wave, gpu_ids, spec_path, lock_path)
    canonical = sum(len(jobs) for _, jobs in plan)
    pending = sum(
        not job.done_file.is_file() for _, jobs in plan for job in jobs)
    print(f"[shell-game-v2-launch] wave={args.wave} canonical={canonical} "
          f"pending={pending} gpus={','.join(map(str, gpu_ids))} "
          f"execute={args.execute}", flush=True)
    if not args.execute:
        for line in preview_lines_v2(plan):
            print(line)
        return
    for wave, jobs in plan:
