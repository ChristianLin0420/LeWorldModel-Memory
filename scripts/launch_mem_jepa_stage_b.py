#!/usr/bin/env python3
"""Launch Mem-JEPA Stage-B age jobs on GPUs 0, 1, and 2."""
...
def main() -> None:
    ...
    for age, gpu in jobs:
        ...
        command = [
            python_bin(),
            str(ROOT / "scripts/run_mem_jepa_stage_b.py"),
            "--age", str(age),
            "--output", str(output),
        ...
        print(f"[stage-b-launch] age={age} gpu={gpu} pid={process.pid}", flush=True)
    failed = []
    try:
        while processes:
            still_running = []
            for age, gpu, process, log in processes:
                ...
                if code is None:
                    ...
                    continue
                log.close()
                print(f"[stage-b-done] age={age} gpu={gpu} code={code}", flush=True)
                if code != 0:
        ...
        for _, _, process, log in processes:
            ...
            log.close()
    if failed:
        raise SystemExit(f"Stage-B jobs failed: {failed}")
    aggregate = [
        python_bin(),
        str(ROOT / "scripts/run_mem_jepa_stage_b.py"),
        "--aggregate",
    ...
    subprocess.check_call(aggregate, cwd=ROOT)
