#!/usr/bin/env python3
"""Launch Mem-JEPA Stage-1 runs on GPUs 0, 1, and 2."""
...
RUNNER = ROOT / "scripts" / "run_mem_jepa_stage1.py"
GPUS = (0, 1, 2)
AGES = (4, 8, 15)
...
def parse_args(argv: Iterable[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument("--output", type=Path,
                        default=ROOT / "outputs" / "mem_jepa_stage1")
    ...
    parser.add_argument("--detach", action="store_true",
                        help="start jobs and return immediately")
...
def aggregate(args: argparse.Namespace) -> None:
    command = [str(PYTHON), str(RUNNER), "--output", str(args.output),
               "--aggregate"]
...
def main(argv: Iterable[str] | None = None) -> None:
    ...
    receipt = {
        "schema": "mem_jepa_stage1_launch_v1",
        "output": str(args.output),
        "jobs": [{"age": job.age, "seed": job.seed, "gpu": job.gpu,
    ...
    while processes:
        ...
        for job, proc in processes:
            ...
            if code is None:
                ...
                continue
            failed += int(code != 0)
            print(f"[mem-jepa] {job.name} gpu{job.gpu} exit={code}",
                  flush=True)
        processes = still
        if processes:
            time.sleep(10)
    if failed:
        raise SystemExit(f"{failed} Mem-JEPA jobs failed")
    aggregate(args)
