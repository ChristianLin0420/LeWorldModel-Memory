#!/usr/bin/env python3
"""Train one age-balanced carrier for adaptive Wave 1.1 color recall."""
...
from scripts.prepare_paper_a_matched_color_v1_1 import (  # noqa: E402
    base_cache_path,
    cue_cache_path,
    host_manifest_path,
)
...
def _load_base(spec: Mapping[str, Any], host: str,
    ...
    expected = {
        "schema": "paper_a_matched_color_v1_1_base_cache_v1",
    ...
    if failed or set(arrays) != BASE_KEYS:
        raise ValueError(f"Wave 1.1 base cache differs {path}: {failed}")
    return arrays
...
def _load_cue(spec: Mapping[str, Any], host: str, split: str,
    ...
    arrays, sidecar = load_verified_npz(path)
    expected = {
        "schema": "paper_a_matched_color_v1_1_cue_cache_v1",
        "study": spec["study"], "lock": spec["_lock"],
    ...
    if failed or set(arrays) != CUE_KEYS:
        raise ValueError(f"Wave 1.1 cue cache differs {path}: {failed}")
    cue_on = 19 - age - 3
    if not np.all(arrays["cue_on"] == cue_on) \
            or not np.all(arrays["cue_off"] == cue_on + 3):
        raise ValueError(f"Wave 1.1 cue timing differs at age {age}")
    return arrays
...
def _aligned_latent(base: Mapping[str, np.ndarray],
    ...
    for key in ("episode_index", "local_start"):
        if not np.array_equal(base[key], cue[key]):
            raise ValueError(f"Wave 1.1 base/cue pairing differs for {key}")
...
def main(argv: Iterable[str] | None = None) -> None:
    ...
    if not args.execute:
        raise SystemExit("refusing Wave 1.1 carrier writes without --execute")
    ...
    if os.environ.get("CUDA_VISIBLE_DEVICES") not in (None, "", "0"):
        raise RuntimeError("Wave 1.1 carrier training requires physical GPU 0")
    ...
    for host in HOSTS:
        ...
        if not path.is_file():
            raise FileNotFoundError(f"Wave 1.1 admission missing: {path}")
        ...
        if value.get("lock") != spec["_lock"] \
            ...
            raise RuntimeError(
                f"global Wave 1.1 admission failed at host: {host}")
        admissions[host] = value
    admission_path = host_manifest_path(spec, args.host)
    admission = admissions[args.host]
    provenance = admission.get("provenance", {})
    fresh = (provenance.get("freshness", {}).get(
        "prior_trajectories_reused") is False if args.host == "reacher"
        else provenance.get("prior_screen_fresh_data_exclusion", {}).get(
            "zero_overlap_proven") is True)
    if not fresh:
        raise RuntimeError(f"Wave 1.1 freshness proof is absent: {args.host}")
    ...
    for split in ("train", "validation"):
        ...
        for age in AGES[1:]:
            if not np.array_equal(
                ...
                raise ValueError("Wave 1.1 joint labels differ across ages")
    ...
    if carrier.parameter_count():
        ...
        for epoch in range(1, epochs + 1):
            ...
            if epoch == 1 or epoch % 10 == 0:
                print(f"[matched-color-v1.1] {args.host}/{args.arm}/s{args.seed} "
    ...
    if state_digest(carrier) != state_before_eval:
        raise RuntimeError("Wave 1.1 evaluation mutated the carrier")
    host_after = state_digest(model)
    if host_before != host_after:
        raise RuntimeError("Wave 1.1 training mutated the frozen host")
    ...
    metrics = {
        ...
        "adaptive_origin": spec["adaptive_origin"][
    ...
    print(f"[matched-color-v1.1] wrote {destination}", flush=True)
