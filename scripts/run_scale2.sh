...
for s in 0 1; do for env in tmaze occlusion recall distractor; do for d in none multi; do
  DJOBS+=("scripts/train_memory.py --env $env --memory-mode $d --seed $s --encoder dino --fixed-alpha --output-dir outputs/dino --wandb-project lewm-memory-dino --extra-tag exp:dino --epochs 20 --num-episodes 2000 --batch-size 16 --num-workers 2 --eval-interval 20 --probe-episodes 96")
