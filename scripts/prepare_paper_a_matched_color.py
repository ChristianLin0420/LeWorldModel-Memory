#!/usr/bin/env python3
"""Prepare fresh adaptive Wave-1b color-only caches and admission receipts."""
...
BASE_SCHEMA = "paper_a_matched_color_base_cache_v1"
CUE_SCHEMA = "paper_a_matched_color_cue_cache_v1"
BASE_KEYS = (
...
def _fresh_hdf_selections(
    ...
    """Select only episodes outside the V1 cache union locked at sealing."""
    ...
    excluded = np.asarray(
        v1_excluded_episode_indices(spec, host), dtype=np.int64)
    if len(excluded) and (excluded.min() < 0 or
                          excluded.max() >= dataset.num_episodes):
        raise RuntimeError(f"{host} locked V1 exclusion is outside dataset")
    if len(excluded) and not np.isin(excluded, eligible).all():
        raise RuntimeError(f"{host} locked V1 cache contains ineligible episodes")
    ...
    if len(available) < requested:
        raise RuntimeError(
            ...
            f"{requested} requests after excluding {len(excluded)} V1 episodes")
...
def _reacher_arrays(spec: dict[str, Any], model: torch.nn.Module,
    ...
    provenance = {
        ...
        "freshness": {
            "wave1_base_seeds": [20260741, 20260742],
            "wave1b_base_seeds": [
                int(source["train_base_seed"]),
                int(source["validation_base_seed"])],
            "seed_pairs_disjoint": True,
...
def _hdf_arrays(spec: dict[str, Any], host: str,
    ...
    if overlap:
        raise RuntimeError(f"{host} overlaps locked V1 data")
    ...
    provenance = {
        ...
        "v1_fresh_data_exclusion": {
            ...
            "locked_v1_indices_sha256": locked_exclusion["indices_sha256"],
            "selected_episode_count": len(selected_indices),
            "overlap_count": len(overlap),
            "overlap_episode_indices": overlap,
            "zero_overlap_proven": len(overlap) == 0,
            "v1_cache_receipts": locked_exclusion["cache_candidates"],
            "v1_host_manifest": locked_exclusion["v1_host_manifest"],
        },
    }
    return base, cue, provenance
...
def main(argv: Iterable[str] | None = None) -> None:
    ...
    if manifest_path.exists() or stop_path.exists():
        raise FileExistsError(f"Wave-1b host branch already resolved: {args.host}")
    try:
        ...
        if host_before != host_after:
            raise RuntimeError("frozen host changed during Wave-1b encoding")
        ...
        payload = {
            ...
            "branch": "adaptive-matched-color-fixed-endpoint",
            "adaptive_origin": {
                "parent_study": spec["adaptive_origin"]["parent_study"],
                "adaptation": spec["adaptive_origin"]["adaptation"],
                "v1_stop_receipt": spec["adaptive_origin"]["v1_stop_receipt"],
            ...
            "paper_appendix_reported_tworoom_history": (
                1 if args.host == "tworoom" else None),
        ...
        print(f"[matched-color] admitted {manifest_path}", flush=True)
    except SystemExit:
        raise
