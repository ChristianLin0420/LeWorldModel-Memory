#!/usr/bin/env python3
"""Run the development-only frozen salience gate for one V3 capacity stage."""
...
def main(argv: Iterable[str] | None = None) -> None:
    ...
    destination = development_cache_path_v3(spec, args.stage)
    paths = (
        destination,
        destination.with_suffix(destination.suffix + ".json"),
        development_receipt_path_v3(spec, args.stage),
        development_manifest_path_v3(spec, args.stage),
    )
    for path in paths:
        if path.exists():
    ...
    if not torch.cuda.is_available():
        raise RuntimeError("V3 development encoding requires an allowed CUDA device")
    ...
    batch, sidecar = load_stage_v3(spec, args.stage, "development")
    audit_receipt = load_counterfactual_audit_v3(
        spec, args.stage, "development", batch, sidecar)
    latents = encode_frames(
        model, batch.frames, device, args.frame_batch_size,
        f"v3-development/{args.stage}")
    ...
    metadata = {
        "schema": "official_shell_game_development_cache_v3",
        "study": spec["study"],
        "stage": args.stage,
        ...
        "source_stage": {
            "path": str(stage_path_v3(spec, args.stage, "development")),
    ...
    fit_inputs = slice_admission_inputs_v3(full_inputs, 0, fit_stop)
    check_inputs = slice_admission_inputs_v3(
    ...
    decision = development_selection_decision_v3(
        diagnostic, float(selection_spec["threshold"]))
    selected = decision["selected"]
    receipt = {
        "schema": "official_shell_game_salience_selection_v3",
        "study": spec["study"],
        "stage": args.stage,
        "display_name": batch.display_name,
        "capacity": batch.contract.stage.capacity,
        ...
        "candidate": V3_SALIENCE.describe(),
        "candidate_count": 1,
        "criterion": {
        ...
        "v2_failed_gate_evidence":
            spec["amendment"]["parent_v2"]["evidence"][args.stage],
        "development_diagnostics": diagnostic,
    }
    receipt_path = development_receipt_path_v3(spec, args.stage)
    receipt_hash = atomic_text(receipt_path, stable_json(receipt))
    manifest = {
        "schema": "official_shell_game_development_manifest_v3",
        "study": spec["study"],
        "stage": args.stage,
        "formal_lock": lock_receipt_v3(spec),
    ...
    atomic_text(
        development_manifest_path_v3(spec, args.stage), stable_json(manifest))
    print(json.dumps({
        "stage": args.stage,
        "selected": selected,
