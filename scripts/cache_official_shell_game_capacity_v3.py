#!/usr/bin/env python3
"""Cache formal frozen features and evaluate unchanged V1/V2 gates for V3."""
...
def main(argv: Iterable[str] | None = None) -> None:
    ...
    for split in FORMAL_SPLITS_V3:
        destination = cache_path_v3(spec, args.stage, split)
    ...
    for path in (admission_path_v3(spec, args.stage),
                 cache_manifest_path_v3(spec, args.stage)):
        if path.exists():
            raise FileExistsError(f"refusing to overwrite V3 artifact {path}")

    batches, sidecars, audits = {}, {}, {}
    for split in FORMAL_SPLITS_V3:
        batches[split], sidecars[split] = load_stage_v3(
            spec, args.stage, split)
        if sidecars[split].get("development_selection") != selection:
            raise ValueError(
                f"formal V3 stage has stale salience evidence: {args.stage}/{split}")
        audits[split] = load_counterfactual_audit_v3(
    ...
    if not torch.cuda.is_available():
        raise RuntimeError("formal V3 caching requires an allowed CUDA device")
    ...
    for split in FORMAL_SPLITS_V3:
        ...
        metadata = {
            "schema": "official_shell_game_cache_v3",
            "study": spec["study"],
            "stage": args.stage,
            ...
            "source_stage": {
                "path": str(stage_path_v3(spec, args.stage, split)),
        ...
        records[split] = write_npz_with_sidecar(
            cache_path_v3(spec, args.stage, split),
    ...
    admission.update({
        "schema": "official_shell_game_frozen_admission_v3",
        "study": spec["study"],
        "stage": args.stage,
        "formal_lock": lock_receipt_v3(spec),
        "amendment": spec["amendment"]["kind"],
    ...
    admission_hash = atomic_text(
        admission_path_v3(spec, args.stage), stable_json(admission))
    manifest = {
        "schema": "official_shell_game_cache_manifest_v3",
        "study": spec["study"],
        "stage": args.stage,
        "formal_lock": lock_receipt_v3(spec),
        "amendment": spec["amendment"]["kind"],
        ...
        "artifacts": records,
        "admission": {
            "path": str(admission_path_v3(spec, args.stage)),
            "sha256": admission_hash,
            "admitted": admission["admitted"],
        },
    }
    atomic_text(cache_manifest_path_v3(spec, args.stage), stable_json(manifest))
    print(json.dumps({
        "stage": args.stage,
        "admitted": admission["admitted"],
        "threshold_changed_from_v1_or_v2": False,
