#!/usr/bin/env python3
"""Fail-closed aggregation for adaptive Wave-1b matched color recall."""
...
def _stratified_weights(joint: np.ndarray, draws: int,
    ...
    if labels.shape != (EPISODES,) or not np.array_equal(
        ...
        raise ValueError("Wave-1b labels are not exactly 16-way balanced")
...
def _load_grid(spec: Mapping[str, Any]) -> tuple[
    ...
    for host_index, host in enumerate(HOSTS):
        ...
        if admission.get("lock") != spec["_lock"] \
            ...
            raise ValueError(f"invalid Wave-1b admission: {host}")
        provenance = admission.get("provenance", {})
        fresh = (provenance.get("freshness", {}).get(
            "wave1_trajectories_reused") is False if host == "reacher"
            else provenance.get("v1_fresh_data_exclusion", {}).get(
                "zero_overlap_proven") is True)
        if not fresh:
            raise ValueError(f"Wave-1b freshness proof differs: {host}")
        ...
        for arm_index, arm in enumerate(ARMS):
            for seed_index, seed in enumerate(SEEDS):
                ...
                if not all(path.is_file() for path in paths.values()):
                    raise FileNotFoundError(f"incomplete Wave-1b cell {directory}")
                ...
                if failed or metrics.get("ages") != list(AGES) \
                    ...
                    raise ValueError(f"invalid Wave-1b cell {directory}: {failed}")
                ...
                if reference is None:
                    ...
                    raise ValueError(f"Wave-1b labels differ across {host} cells")
                ...
                if any(value.shape != (EPISODES,) for value in (
                        ...
                        or not np.array_equal(
                            ...
                            np.full(16, 30)):
                    raise ValueError(f"Wave-1b nuisance deck differs: {host}")
                for age_index, age in enumerate(AGES):
                    ...
                    if current.shape != (EPISODES,) \
                        ...
                        raise ValueError(
                            f"invalid Wave-1b readout {host}/{arm}/{seed}/{age}")
    ...
    if actual != expected_directories:
        raise ValueError("Wave-1b carrier directory set differs")
    audit = {
        "admissions": admissions, "cell_artifacts": cell_records,
        "complete_cells": len(cell_records), "expected_cells": 75,
...
def main(argv: Iterable[str] | None = None) -> None:
    ...
    if not args.execute:
        raise SystemExit("refusing Wave-1b aggregation writes without --execute")
    ...
    if destination.exists() or audit_path.exists():
        raise FileExistsError("Wave-1b aggregation is already closed")
    ...
    summary = {
        ...
        "adaptive_after_v1_location_stop": True,
        "v1_admission_metrics_used_for_inference": False,
        ...
        **analysis,
        "claim_boundary": (
            "Adaptive color-only replication after V1 rejected spatial cue "
            "availability; location is a randomized balanced nuisance and "
            "native host environment/training distribution remains bundled."),
    }
    ...
    print(f"[matched-color] wrote {destination} and {audit_path}", flush=True)
