"""CPU construction tests for the cue-only shell-game V2 amendment."""
...
def test_v2_changes_only_cue_pixels_and_preserves_exact_audit(stage) -> None:
    base = _base(seed=81)
    ...
    for name in (
        ...
        np.testing.assert_array_equal(
            getattr(v2_primary, name), getattr(v1_primary, name))
        np.testing.assert_array_equal(
            getattr(v2_counterfactual, name), getattr(v1_counterfactual, name))

    cue_mask = np.zeros(v2_primary.frames.shape[:2], dtype=bool)
    for episode in range(v2_primary.num_episodes):
        for start, stop in zip(
                v2_primary.cue_on[episode], v2_primary.cue_off[episode],
                strict=True):
            cue_mask[episode, int(start):int(stop)] = True
    np.testing.assert_array_equal(
        v2_primary.frames[~cue_mask], v1_primary.frames[~cue_mask])
    np.testing.assert_array_equal(
        v2_counterfactual.frames[~cue_mask],
        v1_counterfactual.frames[~cue_mask])
    assert np.any(v2_primary.frames[cue_mask] != v1_primary.frames[cue_mask])

    report = audit_paired_counterfactual(v2_primary, v2_counterfactual)
...
def test_v2_cued_cup_and_larger_ball_are_unambiguous_item_color() -> None:
    ...
    for episode in range(v2.num_episodes):
        for item, (start, _) in enumerate(contract.cue_windows):
            ...
            np.testing.assert_array_equal(
                v2.frames[episode, start, contract.ball_y, center + 5], color)
...
def test_v2_retains_semantic_capacity_and_final_endpoint_contract() -> None:
    assert V2_SALIENCE.ball_radius == 6
    assert V2_SALIENCE.describe()["selection_candidates"] == 1
    for stage in CAPACITY_STAGES:
        contract = ShellGameCapacityContract(stage)
        description = v2_contract_description(contract)
        semantic = description["semantic_contract"]
        assert description["semantic_contract_changed_from_v1"] is False
...
def test_development_partition_is_disjoint_and_complete() -> None:
    ...
    batch, _ = paired_counterfactual_batches_v2(
        base, ShellGameCapacityContract(CAPACITY_STAGES[1]), seed=86)
    inputs = build_admission_inputs(batch)
    fit = slice_admission_inputs_v2(inputs, 0, 3)
    check = slice_admission_inputs_v2(inputs, 3, 6)
    ...
    with pytest.raises(ValueError, match="invalid admission slice"):
        slice_admission_inputs_v2(inputs, 3, 3)
...
def test_formal_data_gate_fails_closed_without_development_receipt(tmp_path) -> None:
    ...
    with pytest.raises(FileNotFoundError, match="formal data is blocked"):
        require_selected_salience_v2(spec, "single-item")
...
def test_development_selection_retains_point_seven_five_threshold() -> None:
    ...
    decision = development_selection_decision_v2(diagnostic, 0.75)
    assert decision["selected"] is True
    assert decision["threshold"] == 0.75
    with pytest.raises(ValueError, match="exactly 0.75"):
        development_selection_decision_v2(diagnostic, 0.74)
    ...
    assert development_selection_decision_v2(
