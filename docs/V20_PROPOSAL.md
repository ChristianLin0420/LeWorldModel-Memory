...
## 10. W2 execution: the drift protocol (2026-07-04)

**Status: COMPLETE — 42 deployment evaluations over 3 regimes (fresh 480-episode streams, shift at 240), 0 crashes. Claim 3 PASS (6/6, both regimes). Claim 5 vs fixed-trust FAIL (0/6, both regimes). Claim 5 derived-vs-fixed-η PASS (6/6, both regimes, ≈ +0.15). The stationary guard FAILED on the longer fresh streams. Every verdict has its mechanism in the telemetry.**

Post-shift registered probe (pooled t1dev/t3dev × 3 seeds; pre-shift in parentheses):

| arm | stationary | drift_gap | drift_noise |
|---|---|---|---|
| lkc_rfix | **0.625** (0.607) | **0.636** (0.607) | **0.634** (0.607) |
| dfc (ρ\*=1e-6) | 0.578 (0.599) | 0.598 (0.599) | 0.583 (0.599) |
| acgru | 0.500 (0.469) | 0.490 (0.469) | 0.504 (0.469) |
| dfc_etafix (η\*=1e-3) | 0.430 (0.491) | 0.448 (0.491) | 0.442 (0.491) |
| dfc ρ=1e-4 / ρ=1e-2 (exploratory) | 0.369 / 0.329 | 0.398 / 0.354 | 0.377 / 0.327 |
| none | 0.331 | 0.331 | 0.331 |

**Insight V20.5 — calibration was restored, and calibration was not the bottleneck.** The dual filter does exactly what dual estimation promises: fixed trust turns out *over-cautious* on the drifted streams (post-shift innovation z² ≈ 0.56–0.65 — the model over-estimates its noise), and DFC pulls the ratio back to ≈ 1.12 in **6/6 cells on both regimes** (claim 3 PASS). Yet the probe scores move the *wrong* way (claim 5: 0/6 against fixed-trust on both regimes). The mechanism is now obvious in hindsight: the evaluation family is linear probes on `prior_read`, and a global trust/innovation-scale error is close to an information-preserving reparameterization from a trained linear readout's perspective — the probe just rescales. NLL-optimal recalibration therefore buys ~nothing, while the walk's noise costs a little. This sharpens P3's Insight 10 ("mechanism real, utility not") into the deployment regime, and completes a trilogy with Insights 8/V20.4: calibration pressure — in training, per frame, or at deployment — pays for itself only if something downstream actually consumes calibrated uncertainty. Linear ξ-readouts do not.

**Insight V20.6 — the gain that is safe when stationary is asleep at the shift.** The routing prediction was "η responds only under drift." The telemetry shows the opposite failure: φ velocity ≈ 0.12 in the pre-shift half, ≈ 0 after — at ρ = 1e-6 the parameter-uncertainty budget P is consumed by the cold start (the s-EMA opens empty, η₀ ≈ 0.38) and the walk replenishment is too slow for anything to be left by episode 240. **The derived-gain filter has one time-constant budget, and it spends it where the first surprises are — the start of the stream — not where the drift is.** The pre-registered exploratory arms close the escape route: ρ = 1e-4 and 1e-2 stay awake longer and lose *more* everywhere (diffusion cost > adaptation value at every swept rate). Within this design family the dilemma is structural: waking up for mid-stream drift requires either a P-replenishment rate tuned to the expected drift rate (a hyperparameter this program refuses) or an explicit drift detector (structure it refuses). Registered as the minimal-DFC dead end.

**Insight V20.7 — the derived gain strictly dominates the fixed learning rate; neither beats not adapting.** dfc beats dfc_etafix by +0.150/+0.141 (6/6) under both drifts, +0.12 on dev stationary — uniformly, at every regime tested, the classical per-parameter gain η_t = P/(P+s) is the better weight-space filter than AdaJEPA's constant η. This is the source-kernel-level result and it survives everything: *if* one adapts parameters online in this family, the self-normalizing derived gain is how. But on every stream we built, the best number of parameter updates was zero.
