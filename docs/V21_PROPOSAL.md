...
**Pooled d = +0.996, CI95 [+0.637, +1.899], p_pos = 5.0×10⁻⁵ → CONFIRMED (3/3 tasks positive).** *Health-gate disclosure (found in post-hoc audit, 2026-07-05):* every one of the 30 `gdelta_l10` X1 training cells fails the program's registered health gates — final effective rank 3.0–10.7 against `min_effective_rank: 16`, carrier-k telemetry nan — while `lkc_rfix` passes 27/30; the X1 gate script checks registration timestamps, not health. Two readings, neither currently adjudicated: the rank gate was designed for vector-state carriers and may not transfer to a matrix-state delta cell (its `prior_read` is a 95-dim projection of a rank-limited state — low effective rank may be the mechanism, not a pathology), or the envelope\* is a degenerate trainee and the confirmation is against a sick rival. Since §8's claim-1 sensitivity leaned on these same gates, this asymmetry must ship with the result; the clean resolution is a delta-cell-appropriate health criterion or a rank-passing rival at n=10, registered before any Paper-B submission. The V20 inversion is no longer a moot-clause quantification: on fresh seeds, against the strongest of sixteen fairly-tuned rivals including the modern delta-rule family, under the repaired t4 probe family with t4 *included*, the derived filter wins the registered single-endpoint gate. The baseline-effort-asymmetry explanation is falsified. (Scope note kept honest: the gate is a single registered endpoint at α = 0.05; X2's claims ran under their own registered gates rather than a joint Holm family — the proposal's "Holm-corrected with X2" clause was superseded by the X2 amendment trail, reported as such.) Artifacts: `outputs/v21_x1/x1_gates.{json,md}`.
...
**MIKASA-Robo: deferred with a feasibility note (superseded by §12/F1).** The wheel exists (`mikasa_robo_suite 0.0.5`) but its `mani-skill==3.0.0b15` dependency fails to build in the pinned program environment (numpy source-build failure); the integration path is a dedicated venv plus an EpisodeBatch bank bridge, ≈ 0.5–1 day with GPU-sim risk. Per the review panel's own leverage ranking (last) and the registered A/B split, the external-benchmark arm belongs to Paper B; Paper A's portability evidence is the two-host s\* + delay scaling above.

---

## 12. Paper-A fix wave — registrations (2026-07-05, written before any result)

Per `docs/V21_SUBMISSION_REVIEW.md` fixes 1–3. Each item's confirmed-if is frozen here first; results are appended below their registrations as they land.

**F1 — MIKASA-Robo RememberColor through the certification pipeline (GPU 0).** Dedicated venv (`.venv-mikasa`, program env untouched); EpisodeBatch bridge from `mikasa_robo_suite` RememberColor episodes under xi-independent scripted actions; frozen DINOv2 features as the certified-sighted host (per §11, no training anywhere). Registered readouts: (a) sighted certificate — registered P1b probe on cue-window features, pass ≥ 0.75; (b) memory-demand certificate — integrator-features floor (first frame + action prefix) within 0.05 of chance, and the no-leakage clause: a decision-window frame probe also within 0.05 of chance (xi must not be readable anywhere after the cue) — *amended 2026-07-05 before any F1 result existed: the clause originally read "post-cue probe clears 0.75", which is the wrong sign (a decision-time probe clearing 0.75 would prove the absence of demand); corrected to the program's standard leakage check*; (c) inversion direction — lkc_rfix vs gdelta_l10 carriers trained on frozen DINO features, n = 5 seeds, registered probe family, direction-only endpoint (no p-value at this n). *Any* honest outcome is the deliverable, including an informative certification failure (a memoryless shortcut in an external benchmark is a finding). Feasibility failure clause: if the dependency stack cannot build tonight, the full build log is the artifact and the arm reports infeasible-as-packaged.

**F2a — localize s\*(DINOv2) (GPU 1).** The §11 ladder is left-censored (DINOv2 ≥ 0.996 everywhere). New dev-only rungs below s1, keeping the fill-tiles-ring invariant `cue_half = marker_radius − 1` and the s-family geometry: `t1s0a` (radius 3, half 2, border 0), `t1s0b` (radius 2, half 1, border 0), `t1s0c` (radius 2, half 1, border 0, duration (2,3) — the shortest-lowest rung). Same certificate (3 bank seeds, majority, ≥ 0.75). Confirmed-if: some rung fails → s\*(dino) is a localized threshold with the censoring removed. If all pass, the censoring stands and is reported as such (the instrument bottoms out at this render resolution).

**F2b — the deconfound: same recipe, second scene (GPU 1).** `dmc_base` gains a domain parameter (default `reacher`, frozen paths unaffected); the t1 salience family renders its overlays on **cartpole** — a scene no task was ever tuned on. Registered readouts: (a) frozen DINOv2 ladder on cartpole banks (eval-only); (b) the program's VICReg host recipe trained on the cartpole-s1 stream (W1 recipe, none-carrier, 2 seeds), then sighted-certified on its own stream's ladder. Interpretation table frozen now: trained-VICReg fails low rungs on cartpole too while DINOv2 passes → the deletion is a property of the SSL-training regime, reproduced across scenes (the §11 "acquired, not architectural" attribution deconfounded from the reacher scene); trained-VICReg passes on cartpole → the deletion is reacher-specific and §11's attribution is struck to a single-scene observation. Either way the certificates run on a second scene, which no prior generation achieved.

**F3 — the retention horizon as a design knob (GPU 2).** The delay-scaling caveat (§11) stands as measured; the registered question here is whether the filter's decay is *design-controllable* where the learned carriers' is not. Intervention: on frozen lkc_rfix checkpoints, re-derive the fixed spectrum with TAU_RANGE scaled by r = L/64 per evaluated length (the hold channel untouched, all learned weights frozen), evaluate on the §11 delay banks. Confirmed-if: rescaled ≥ frozen + 0.05 mean probe score at L = 128 (3 checkpoint seeds) with no loss (> −0.02) at L = 64. Falsified-if: rescaling does not help → the delay decay is not spectrum-limited and the "derived carrier" framing loses its horizon-control selling point (reported as such).

**F3 result (2026-07-05): NOT MET — the registered falsified-clause fires, and it sharpens the delay story.** Rescaled-vs-frozen deltas: +0.000 (L=64, the r=1 no-op sanity check, exact), +0.018 (L=96), +0.002 (L=128) — far under the +0.05 bar (`outputs/v21_x3/tau_rescale.{json,md}`). The delay decay is **not spectrum-limited**: matching every finite half-life to the evaluated horizon recovers nothing, which is consistent with the mechanism — the cue estimate rides the eigenvalue-1 hold channel, which never decayed in the first place. The extrapolation loss therefore lives in the length-64 statistics the learned weights (readout, gain, trust) were fit under, not in state retention. Paper framing: the horizon-control selling point is dead as registered; in exchange the §11 caveat gets a mechanism ("the filter retains; the trained readout is length-bound") — reported exactly so, as a falsification with an explanation, not respun as support.

**F1 result (2026-07-05): the external arm ran, and the certificate caught a memoryless shortcut in the external benchmark.** The three-generation blocker was a `numpy==1.23.5` pin with no py3.12 wheels; the dedicated venv (`.venv-mikasa`, modern numpy, deps installed around the pin) builds and simulates. RememberColor9-v0, 3 bank seeds × (512 train + 256 eval) episodes, xi-independent random actions, frozen DINOv2 host (`outputs/v21_f1/certification.{json,md}`):

| readout | result | verdict |
|---|---|---|
| sighted (cue window) | 1.000 / 1.000 / 1.000 | PASS |
| no-leakage (decision window) | 0.086–0.125 (chance 0.111) | PASS |
| integrator floor | **0.875 / 0.930 / 0.859** | **FAIL — floor ≫ chance** |

The floor fails because the cue cube is visible at t = 0: `enc(o_0)` plus the action stream decodes the color at ~0.9, so under the program's two-sided definition RememberColor9 demands **initial-observation storage, not online filtering** — a reactive policy conditioned on its first frame needs no belief state. (MIKASA's own demand notion is sliding-window observability, under which the task is memory-demanding; the certificate distinguishes the two demand types, and that distinction is the finding.) This is the registered informative-failure outcome: the instrument's first contact with a world it did not build produced a substantive audit result for Paper A's central claim — *benchmarks labeled "memory" can be certified, and the certificate finds structure their authors' framing misses*.

**F1 inversion direction: positive on the external family.** Carriers trained on frozen DINO features (transfer recipe: next-feature prediction; documented deviation from the W1 joint objective), registered probe family (gap-window mean ++ t_dec read), n = 5, direction-only as registered: **lkc_rfix 0.845 (per-seed 0.840–0.852) vs gdelta_l10 0.600 (per-seed 0.309–0.879), direction +0.245.** Two externally-replicated program observations in one table: the filter leads, and the delta cell is a seed lottery (sd ≈ 0.23 vs the filter's 0.004) — the same instability its X1 health-gate failures and X2 chance-level selector seed flagged internally. Labeled exploratory (transfer recipe, n = 5, no registered statistic).

**F2a result (2026-07-05): the censoring stands — and widens the two-host gap.** All three sub-s1 rungs PASS at ≥ 0.988 (t1s0c: a 2-px-radius disc lasting 2–3 frames — the smallest, shortest cue this scene can meaningfully render): `outputs/v21_x3/dino_sstar_ext.{json,md}`. s\*(dino) remains left-censored at the render floor, reported per the registration's fallback clause: the frozen backbone's salience range exceeds the scene's, so the *threshold* is not localizable on this instrument — but the *ordering* statement strengthens from one rung to ≥ three: s\*(dino) ≤ t1s0c < t1s1 < s\*(vicreg) = t1s2. The honest §11 correction: "s\* is a two-host measurement with a demonstrated ordering" stays; "threshold" language for the DINO side becomes "upper bound".

**F2b result (2026-07-05): the deconfound fires the correction branch — the deletion is scene-specific, and §11's attribution is struck as registered.** Scene amendment first (made before results existed): cartpole has 1-dim actions and breaks the frozen ACTION_DIM = 2 contract; the second scene is **point_mass** (2-dim, visually disjoint from reacher), via an env-var domain override that leaves every frozen path byte-identical. The two-host × two-scene sighted grid (`outputs/v21_x3/dino_sstar_pointmass.{json,md}`, `outputs/v21_f2b/`):

| host \ scene | reacher | point_mass |
|---|---|---|
| frozen DINOv2 | ≤ t1s0c (censored, ≥ 0.988 everywhere) | ≤ t1s0c (censored, 1.0 everywhere) |
| task-trained VICReg (W1 recipe) | s\* = t1s2 — **fails s1** (0.297–0.746) | **passes s1** (0.871 / 0.789; floor at chance; health gates PASS) |

The task-trained recipe reads on point_mass the same low-salience cue it deletes on reacher. Per the frozen interpretation table, "encoder blindness is acquired, not architectural" is **withdrawn as a mechanism claim** and rescoped: the deletion is a *scene-dependent* property of training on the stream — present where the endogenous scene competes for the variance budget (reacher's arm), absent on a sparse scene (point_mass) — while the frozen general-purpose backbone is above the render ceiling on both scenes. What survives, now on two scenes: the per-task sighted certificate is the only reliable way to know which regime a given (encoder, scene) pair is in — which is the instrument thesis, strengthened by its own correction. Forward registration: the variance-budget-competition hypothesis needs a scene-complexity sweep to become a claim; until then it is a hypothesis in an appendix.
