...
**F1 — MIKASA-Robo RememberColor through the certification pipeline (GPU 0).** Dedicated venv (`.venv-mikasa`, program env untouched); EpisodeBatch bridge from `mikasa_robo_suite` RememberColor episodes under xi-independent scripted actions; frozen DINOv2 features as the certified-sighted host (per §11, no training anywhere). Registered readouts: (a) sighted certificate — registered P1b probe on cue-window features, pass ≥ 0.75; (b) memory-demand certificate — integrator-features floor (first frame + action prefix) within 0.05 of chance while a post-cue-window probe at decision time clears 0.75; (c) inversion direction — lkc_rfix vs gdelta_l10 carriers trained on frozen DINO features, n = 5 seeds, registered probe family, direction-only endpoint (no p-value at this n). *Any* honest outcome is the deliverable, including an informative certification failure (a memoryless shortcut in an external benchmark is a finding). Feasibility failure clause: if the dependency stack cannot build tonight, the full build log is the artifact and the arm reports infeasible-as-packaged.
