...
*Design and experiment record. Branch: `learnable-memory`. Implementations through HACSSM-v9/LOIF live in `lewm/models/memory.py`. Status as of 2026-06-29: every reported V1–V8 experiment is complete. V8 ran all 325 cells, 65,000 W&B epochs, and 325 rollout artifact/table/video bundles; its immutable pilot is `NO_GO` and final label is `PILOT_NO_GO_FINAL_DESCRIPTIVE`. Compact V8 ranks second by paired reduction (+6.20% versus SSM), but loses .23% to V7 shared-action, loses .96% to its retrained endpoint envelope, wins 0/5 task envelopes, and is not compact-noninferior under the frozen bootstrap (§7.10). LOIF-v9 (§2.8/§7.11) is now implemented with learned ordered poles, evidence-conditioned gains, inverse-scale fusion, and one unweighted next-latent objective. Its official 325-cell output namespace remains clear; only an excluded two-epoch engineering smoke has run, so there is no V9 performance result yet.*
...
*Figure 1. Consolidated map of the V1→V9 evolution, architecture-changing controls, every completed V5–V8 mechanism variant, the implemented V9 falsification map, and external/historical families. Seeds, optimizer settings, mask placements, and ordinary hyperparameter sweeps are intentionally omitted. V5–V8 are completed experiments; V9 is visibly marked prelaunch and has no official result.*
...
| **HACSSM-v9 / LOIF** | implemented learned ordered stable poles + evidence-conditioned observation gains; no horizon auxiliary | inverse-scale fusion of two posterior states | 34,563 | prelaunch: code/protocol/smoke complete, no official run or metric; no fixed retention after initialization and no memory-objective weight |
...
*Figure 5e. Implemented post-V8 architecture and prelaunch protocol, not a performance result. LOIF retains the two V8 mechanisms that remain large and stable—one shared action transition and a joint two-state read—but replaces fixed `τ={2,8}`, global shrinkage coefficients, and route logits by learned ordered stable poles, one causal evidence scale, Kalman-inspired gains, and inverse-scale fusion. The controls make each change falsifiable. Section 7.11 records the frozen study contract.*
...
The nominated objective is one unweighted causal next-latent MSE averaged over the eligible visible targets of each episode (`first_post_loss_weight=0` in the legacy configuration). Every SSM/V7/V8 reference in the V9 grid is retrained under that same objective; sealed historical checkpoints are evidence and provenance anchors, not objective-mismatched competitors. LOIF itself has no memory teacher, target horizon set, counterfactual window, hidden-clean target, teacher momentum, auxiliary schedule, or memory-loss coefficient. The retrained V7 shared-action reference necessarily retains its architecture's EMA teacher and `hier_*` diagnostic receipts, but its configured and effective hierarchical weight is exactly zero throughout, so no teacher-derived target contributes a loss or gradient. Under MSE, `P/R` are operational positive scales—not calibrated posterior variances or probabilities—and “Kalman-inspired” describes the algebra only. A likelihood-calibrated model would be a different, separately frozen study and is outside the V9 grid.
...
3. **Selectivity and mechanism ablations — complete through V8; V9 prelaunch.** Router mass matching, causal gate replacement, static/dynamic and old/new-recurrence controls, hard visibility, shifted masks, L0 cardinality, action/no-aux/single-level controls, memory-only action interventions, V5 rate/boundary controls, V6 objective controls, V7 action/shrinkage/recovery controls, and V8 action-tying/compaction/retrained-endpoint controls are complete (§7.5–7.10). A competitive Mamba-style learned-`Δ` baseline remains undone. The eight V9 modes are implemented, but their official grid has not launched.
4. **Router/update/action diagnostics — complete through V8; frozen for V9.** SMT-v2 gates are nearly static (§5); V3 gates are black-sentinel selective (§7.5). V4–V8 repeatedly establish a causally used action path and joint read. V8 supplies the cleanest current estimates (+14.87% and +9.34%), while finding only +.16% for parameter-matched action tying and a −.96% loss to the retrained shrinkage endpoint envelope (§7.10). V9 now logs evidence conditioning, nominal direct retention, action influence, saturation, interventions, and streaming equivalence (§7.11); no official value exists yet.
...
12. **Learned ordered innovation V9 — implemented, official study pending.** LOIF removes fixed post-initialization retentions, shrinkage, route logits, and memory-specific auxiliary weights in favor of learned ordered poles and causal positive-scale updates. Core, diagnostic, protocol, analyzer, legacy-regression, and excluded two-epoch smoke checks pass; the clean 325-cell W&B/rollout study remains unlaunched. Direct controls and a held-out confirmation path are frozen in §2.8/§7.11.
...
**Status: implemented prelaunch architecture; no official result.** The LOIF class, eight modes, trainer diagnostics, runner, analyzer, and tests are complete. One excluded two-epoch Reacher engineering smoke verified finite training, donor interventions, rollout serialization, and batch-size-one streaming; it is not a study cell and its metric is not evidence. A first official invocation at producer `0c8d6dd` stopped before launching any cell or W&B run because the freshly written protocol converted one donor-strata tuple to a JSON list and failed its immediate in-memory equality check. The invalid namespace was archived with protocol SHA-256 `df7ae236820becb1c7331a87d43c911ed1749f244877a9a6a9963b9c80f73cad`; the contract now uses a native list and has an explicit JSON-round-trip regression test. `outputs/hacssm_v9_shared` remains clear. Because V9 was selected after opening V8, the forthcoming score on these same five tasks is adaptive development and cannot be the paper's untouched confirmation.
...
The pilot/final screen is frozen before official launch. A row applies separately to every reference named in that row; reductions are paired on `clean_mse_first_post` and lower is better.
...
For an ICLR claim, the better path after this adaptive-development grid is to freeze any surviving LOIF checkpoint/protocol and evaluate a separately held-out cohort: new rollout seeds; randomized gap positions and lengths; at least one unseen corruption family rather than the exact black sentinel; simulator-state prediction or a state probe; and executed task return/success with tuned recurrent/SSM/Transformer controls. V9's value is that the hierarchy can be rejected cleanly: if learned poles and evidence-derived gains do not beat their direct controls, fixed/simple filtering remains the supported answer.
...
7. Treat completed V7/V8 and the forthcoming V9 grid strictly as adaptive-development evidence. LOIF-v9 is implemented and frozen before its official launch; if it survives, evaluate the unchanged design on unseen rollout seeds, gap distributions, and corruption families plus simulator-state prediction and executed return. Another architecture search on the same five means is not confirmation.

**Recommendation.** The V3–V8 prospective studies all return negative pilot/final labels. V8 is the strongest nominated model versus SSM (+6.20%) and the second paired-average design, but it loses to V7 shared-action, fails noninferiority/equivalence/endpoint-envelope gates, and wins no task envelope. Implemented LOIF-v9 has no official result yet and is not a reason to revise that evidence. The expected main-track outcome remains rejection on task validity, baseline strength, and novelty/positioning. Do **not** submit the current version to the ICLR main track. Either (a) require V9 to survive its frozen adaptive screen and then run untouched corruptions/seeds, state and executed-return outcomes, uncertainty, and tuned baselines, or (b) submit a narrower workshop paper centered on static V2, black-sentinel V3, action-sensitive V4–V8, and the repeated failure of hierarchical self-supervision.
