# Learnable memory for LeWorldModel: SMT-v1–v3 and HACSM/HACSSM/ORBIT/KDIO/SIRO/CF-HIRO/CF-EBO-v4–v14

*Design and experiment record. Branch: `learnable-memory`. Implementations through KDIO-v11 live in `lewm/models/memory.py`; SIRO-v12, CF-HIRO-v13, and CF-EBO-v14 are isolated in `lewm/models/siro.py`, `lewm/models/cf_hiro.py`, and `lewm/models/cf_ebo.py` and integrated through the common LeWM host. Status as of 2026-06-30: every reported V1–V9 experiment is complete. V9 ran all 325 cells, 65,000 W&B epochs, and 325 rollout artifact/table/video bundles; its immutable pilot is `NO_GO` and final label is `PILOT_NO_GO_FINAL_DESCRIPTIVE`. Full LOIF improves 2.55% over its freshly trained SSM, ranks 7/13, loses 4.15% to retrained compact V8 and 4.99% to the V8 endpoint envelope, and wins 0/5 task envelopes. Learned poles lose .67% to fixed poles and the two-bank model loses 2.22% to its single-bank control. Compact V8 is the strongest freshly retrained V9-grid design at +6.38% versus SSM; the earlier sealed V8 cohort instead ranked V7 shared-action first and compact V8 second, so the cohorts are reported separately (§7.10–7.11). **V10-J fixes causal representation collapse but fails its five-task state-quality/convergence screen, so its official grid remains unlaunched at 0/225 (§2.9/§7.12). V11 is the adaptive kick–drift/OAS response. Its mandatory 32-cell V11b development screen is complete and negative, leaving the official V11 ledger at 0/240 pilot and 0/400 final (§2.10/§7.13). SIRO-v12 then completes all 28 adaptive W&B/rollout bundles but loses retrained KDIO-v11 and its legal integrator, fails seven rank checks, collapses algebraically to one dynamic state, and has negative held-fold action partial `R²` on every V12b task; its outcome is `SCREEN_NO_GO / NO_100E_LAUNCH` (§2.11/§7.14). CF-HIRO-v13 has now completed its frozen 36/36-cell W&B/rollout screen and is also negative: full V13 scores `3.965301` equal-task held-out prior NMSE, `nocorrect` is the strongest V13 mode at `.816999`, and freshly retrained raw-difference KDIO-v11 is best overall at `.564411`. Integrity and all registered numerical checks pass, but representation rank, external and internal performance, held-fold action generalization, direct-sum energy, and convergence gates fail. The label is `SCREEN_NO_GO / NO_100E_LAUNCH`; the conditional 100-epoch continuation remains unlaunched at 0/72 (§2.12/§7.15). CF-EBO-v14 is the adaptive response to those failures, but it is only a prospective implementation with frozen launch machinery: status `READY_NOT_RUN`, screen 0/40, online W&B/rollout evidence 0/40, and conditional continuation 0/96. Its implementation-only smoke checks are excluded from every empirical comparison (§2.13/§7.16).***
...
**Empirical verdict.** SMT-v2 does not support the selection hypothesis: its gates are almost static and can be replaced by calibration means without hurting the saved models (§5). L0 routing finds either a dense model or a quality-destroying closed/static subset, and the strongest OC-SMT result uses all 28 banks (§9). SMT-v3 fixes the erasing write rule and learns a causally important black-sentinel gate (§7.5). HACSM-v4 then fixes action blindness: swapping only memory-path blackout actions raises first-post MSE by 12.46%, and V4 beats V3 by 12.49% across paired cells (§7.6). V5's learned channel spectrum and boundary auxiliary regress (§7.7); V6's dense consistency adds only .39% and loses to static correction (§7.8); V7's recovery objective adds only .30% and loses its shared-action/no-recovery controls (§7.9). V8 cleanly removes those special objectives and confirms the durable mechanisms—action transport +14.87%, joint two-state read +9.34%—but the compact/shared-action novelty does not clear its controls. Its learned global shrinkage is interior and beats each endpoint on average, yet loses .96% to selecting the better retrained endpoint per cell and is 1.21% worse than SSM in deep blackout (§7.10). V9 confirms that causal evidence conditioning and action transport are active—full LOIF beats global resistance by 30.56% and no-action by 8.29%—but rejects its intended hierarchy: fixed poles beat learned poles, one slow bank beats two banks, the sample-specific resistance interventions are sub-.5%, and full LOIF remains behind V7/V8 (§7.11). V10's causal host repair preserves rank, but its rotation-only prior loses to a simple action integrator on all five tasks and its original cross-coordinate probe is partly mis-specified (§7.12.6). V11 tests a mechanics-shaped configuration/velocity hierarchy, learned-scale Stiefel action transport, drift, clean OAS reliability, live observed-anchor suffix prediction, and a separate detached executed-versus-deranged rank path. V11b confirms that the action intervention is implemented—`noaction` has exact zero effects and chance ranking—but not that the nominated composition is competitive: only Cartpole has both positive rank advantage and above-chance pair accuracy, the default is 18.44% worse than its legal integrator by mean taskwise paired relative change, and all 32 curves worsen late. The mandatory screen is therefore `SCREEN_NO_GO`, and the official study is `OFFICIAL_NO_LAUNCH`, not a pending or positive V11 result. SIRO-v12 conserves the initial condition, identifies stable dynamics/action transport/correction from synchronized train-only views, removes every memory-specific loss, and audits an all-lag reachability spectrum. The completed screen rejects the proposed composition: full SIRO is 65.80% worse than freshly retrained KDIO-v11 and 99.07% worse than its legal integrator by aggregate held-out NMSE; its online action effect is numerically nonzero but the suffix advantage averages only `5.81e-5`; and the independent audit finds seven representation-rank failures. The later V12b replay exposes the deeper causes—history-fitted LMMSE explodes under recursive deployment, current `A` is nonnormal, normal stable dynamics improve the train-only recursive prior, and action partial `R²` is negative on all tasks. V12 is therefore `SCREEN_NO_GO / NO_100E_LAUNCH`, not evidence of a generally superior hierarchy, calibrated uncertainty model, automatic memory sizing, architectural novelty, or successful ICLR method. CF-HIRO-v13 fixes nonnormality and the ambiguous stream decomposition, but its completed screen isolates a different failure: the fixed Riccati correction amplifies off-fit Gaussian innovations so strongly that `nocorrect` is the best V13 mode, fold agreement does not transfer to held-fold action prediction on 3/4 tasks, and Walker has no measurable output complement. KDIO and the legal integrator remain stronger, so V13 is also `SCREEN_NO_GO / NO_100E_LAUNCH` (§7.15). CF-EBO-v14 is a prospective repair—opposite-fold positive-part empirical-Bayes reliability for both action and correction, a unit-energy correction cap, radial innovation suppression, and machine-cutoff support with exact-zero padding in the fixed V13-sized schema—but it has no screen result yet. It must not be called better than V13, an overall best performer, or an ICLR-ready method while its ledger remains `READY_NOT_RUN` at 0/40 (§7.16).

## 2. The architecture

![Architecture map of tested memory designs](figures/fig_memory_versions_arch.png)
*Figure 1. Consolidated map of the V1→V14 evolution and architecture-changing controls. V12 and V13 are completed negative adaptive screens. V14 is the prospective cross-fold-calibrated energy-bounded observer: it retains V13's fold-agreement normal coordinate, makes its energy support rank-aware, replaces deployed action/correction confidence by opposite-fold risk calibration, and replaces fixed Riccati correction by OAS-whitened, spectrally capped, radial evidence correction. V14 is `READY_NOT_RUN`: frozen screen 0/40 and conditional continuation 0/96; smoke checks are excluded.*
...
### 2.13 CF-EBO-v14: cross-fold-calibrated energy-bounded observer (`READY_NOT_RUN`)

![CF-EBO-v14 architecture, energy certificate, controls, and frozen prospective protocol](figures/fig_cf_ebo_v14_arch.png)
*Figure 5j. Prospective CF-EBO-v14 contract. A pooled V13 real-normal realization defines the source coordinate, an infinite-horizon observability Gramian converts its machine-active subspace into energy coordinates, and inactive directions are exactly zero-padded to retain the fixed V13-sized host schema. Even/odd train episodes calibrate action and correction by opposite-fold positive-part empirical-Bayes risk; pooled maps are deployed. OAS-whitened innovations pass through a unit-spectral correction map and rank-derived radial gate. Six candidate modes and four fresh baselines form the frozen 40-cell screen. The conditional 96-cell continuation is command-complete but unauthorized and cannot be launched by the screen runner. Status is `READY_NOT_RUN`: 0/40 screen cells, 0/40 W&B/rollout bundles, and 0/96 continuation cells. No smoke value is a V14 result.*

V14 is a targeted adaptive response to the completed V13 failure, not a result. It asks whether V13's useful numerical property—stable normal dynamics—can be retained while changing its two unsupported deployed mechanisms: the pooled coordinate still uses V13 fold agreement, but action and correction confidence are calibrated by scoring each fold-specific fit on the opposite fold; the unconditional Riccati gain is replaced by a correction whose future-read energy is bounded and whose response decreases for large whitened innovations. It also makes the output complement and state support explicitly rank-aware. None of those changes establishes superiority before the frozen evidence screen runs.
...
- **A hierarchy is a falsified-or-supported mechanism, not an assumption.** SMT-v1–v3 can be stacked by depth; V4 carries three belief levels; V5–V8 carry two; and V9 carries two learned poles, but V9's one-bank control wins. V10 therefore uses one state. V11 reintroduces exactly two coupled roles—configuration and velocity—and its development controls reject the full composition. V12 nominally stores three causal roles, but full SIRO's identity read and shared `A` make `r+u` an exact sufficient dynamic state. V13's normal blocks have data-derived radii/phases and its complement is algebraically orthogonal to the dynamic read, but the completed screen does not support a useful modal hierarchy: held-fold action generalization fails on three tasks and Walker's complement vanishes. V14 treats zero complement codimension and inactive energy directions as valid rather than forcing a hierarchy; whether its machine-active modes are useful is precisely an unrun screen question.
- **Streaming counts are not measured training peaks.** V1–V9 need `K·D` state (plus V9's two scales), V10 needs `D`, V11 needs `2D`, and the implemented V12 API stores `3D`; full V12 is algebraically reducible to `2D` (`c` plus `r+u`). V13 and V14 store a fixed `n+D` streaming schema and no covariance, even when V14's active support rank is below `n`. Their training implementations retain encoder activations, scans, paired detached views, all-lag Hankels, reconstructed states, and dense FP64 fit tensors. The experiment records peak VRAM, epoch time, fit time, and checkpoint payload before making an efficiency claim; V14 has no such measured screen record yet.
...
17. **Cross-fold-calibrated energy-bounded observer CF-EBO-v14 — `READY_NOT_RUN`.** The prospective implementation retains the pooled V13 normal coordinate but uses a machine-cutoff observable-energy support, exact-zero fixed-schema padding, opposite-fold positive-part empirical-Bayes action/correction reliability, OAS innovation whitening, a unit correction cap, and a rank-derived radial gate. Its frozen screen is 0/40, its online W&B/rollout ledger is 0/40, and its conditional continuation is `CONDITIONAL_NOT_AUTHORIZED` at 0/96. Seed-`14000` implementation smokes are excluded and provide no performance result (§2.13/§7.16).
...
#### 7.15.6 Successor rationale—implemented prospectively, still not a V14 result
...
CF-EBO-v14 now implements that rationale with positive-part empirical-Bayes directional minima, an observable-energy coordinate, a unit-spectral correction cap, a rank-derived radial gate, and exact-zero support padding (§2.13). It directly targets V13's three empirical failures—agreement without opposite-fold action value, Gaussian correction amplification, and an absent complement—but remains a composition of known predictive-state, robust-filtering, and rank-selection ideas. A materially stronger end-to-end LeWM novelty claim would additionally require differentiating through, or using an implicit gradient for, the fitted realization/filter so encoder features are shaped by predictive-state identification rather than merely refitted after each epoch. Such a change would need a new frozen protocol, unopened confirmation data, competitive returns, and explicit controls. V14 is `READY_NOT_RUN`; there is **no automatic V14 launch** and no successor performance result in this document.

### 7.16 CF-EBO-v14 prospective ledger — `READY_NOT_RUN`

This section is reserved for the frozen V14 screen. It contains the architecture and protocol state only; it does **not** contain an empirical V14 result. The implementation, runner, analyzer, independent read-only auditor, and figures exist, but the evidence runner has not been launched from a clean pushed commit.

| evidence ledger | expected | present | status |
|---|---:|---:|---|
| frozen 30-epoch screen cells | 40 | 0 | `READY_NOT_RUN` |
| online W&B epoch histories | 40 | 0 | none |
| rollout NPZ/table/video bundles | 40 | 0 | none |
| analyzer decisions | 1 complete 40-cell grid | 0 | `NOT_EVALUATED` |
| independent read-only audits | 1 complete 40-cell grid | 0 | `NOT_EVALUATED` |
| conditional 100-epoch cells | 96 | 0 | `CONDITIONAL_NOT_AUTHORIZED` |
| official/confirmation V14 result | — | 0 | none; adaptive development only |

#### 7.16.1 Reserved screen-result slots

The following cells must remain unfilled until `scripts/analyze_cf_ebo_v14_screen.py` validates one complete immutable screen and `scripts/audit_cf_ebo_v14_screen.py` independently reproduces it. Values from partial runs, local smoke directories, logs, or manual recomputation do not belong here.

| design | Cartpole | Fish | Pendulum | Walker | equal-task held-out prior NMSE |
|---|---:|---:|---:|---:|---:|
| full CF-EBO | — | — | — | — | — |
| `nocorrect` | — | — | — | — | — |
| `noaction` | — | — | — | — | — |
...
| `noenergycap` | — | — | — | — | — |
| `noradial` | — | — | — | — | — |
| CF-HIRO-v13 `nocorrect` | — | — | — | — | — |
| SSM | — | — | — | — | — |
| compact V8 | — | — | — | — | — |
| raw-difference KDIO-v11 | — | — | — | — | — |

| frozen decision component | current value |
|---|---|
| artifact/provenance integrity | `NOT_EVALUATED` |
| representation gate | `NOT_EVALUATED` |
| numerical/streaming/exact-mode gate | `NOT_EVALUATED` |
| observable-energy support identity | `NOT_EVALUATED` |
| external performance gate | `NOT_EVALUATED` |
| internal-control gate | `NOT_EVALUATED` |
| action/correction opposite-fold mechanism gate | `NOT_EVALUATED` |
| corruption-shift/radial-safety gate | `NOT_EVALUATED` |
| rank-aware complement policy | `NOT_EVALUATED` |
| convergence gate | `NOT_EVALUATED` |
| conjunctive screen decision | `READY_NOT_RUN` |

#### 7.16.2 Frozen provenance and launch boundary

The intended screen uses branch `learnable-memory`, seed `14001`, 30 epochs, four task-pinned GPUs, online W&B study `hacssm-v14-screen-cfebo30`, output root `outputs/hacssm_v14_screen_cfebo30`, and the 10 designs in §2.13.5. The launcher requires a clean worktree with `HEAD==@{upstream}`, then freezes full commit, upstream, source, data, and command hashes. It refuses to reuse any reserved evidence filename. Each official row must have a complete epoch sequence, checkpoint, metrics, rollout artifact, W&B run receipt, and finite operator payload.

The continuation command ledger reserves exactly eight designs, four tasks, seeds `14002/14003/14004`, and 100 epochs: 96 cells under study `hacssm-v14-continuation-cfebo100`. Its present and only valid state is `CONDITIONAL_NOT_AUTHORIZED`, 0/96. The screen runner cannot execute it. The analyzer may write `contingent_100e_launch_manifest.json` only after every registered conjunct passes; that manifest still records authorization rather than launching a process.

#### 7.16.3 Excluded pre-freeze Pendulum development
...
#### 7.16.4 Support-rank repair and other implementation smoke
...
#### 7.16.5 Prospective interpretation rule

At 0/40, V14 has no performance, mechanism, robustness, convergence, or ICLR-readiness conclusion. Its only current claim is that the prospective system is implemented and testable under the frozen contract in §2.13. A future positive screen would authorize the separately controlled continuation, not prove general superiority; a negative screen must be closed as `SCREEN_NO_GO` without launching those 96 cells. The method remains a composition rather than fundamental architectural novelty, uses pooled final refits rather than full cross-fitting, and alternates detached identification with end-to-end RGB training rather than differentiating through identification.
...
15. V14 is a **prospective adaptive repair, not evidence**. Its code and frozen 40-cell/conditional 96-cell machinery are implemented, but no official cell, W&B history, or rollout bundle exists. The correct label is `READY_NOT_RUN`, with screen 0/40 and continuation `CONDITIONAL_NOT_AUTHORIZED` at 0/96. Machine-rank support smokes are implementation-only and excluded; cross-fold-calibrated pooled refits must not be described as fully cross-fitted (§2.13/§7.16).

That is a coherent **diagnostic study**. It is not the broad “learnable selective memory for world models, robotics, and closed-loop control” paper implied by the present title, abstract, and result language. V8 is itself an adaptive response selected on the same development tasks, V9 is selected after opening V8, V10-J is an adaptive host repair whose ORBIT comparisons remain unrun, V11 is an adaptively designed successor stopped by its mandatory development screen, completed-negative V12 and V13 are further adaptive successors evaluated on the already opened four-task caches, and V14 is another adaptive proposal on those same caches with no result yet (§7.10–7.16).
...
7. Treat V7–V14 strictly according to scope. V7–V9 have immutable negative labels. V10-J is an adaptive representation-validity audit with a `NO LAUNCH` decision; V11b is an excluded adaptive `SCREEN_NO_GO`, not a pilot, and its official grid is `OFFICIAL_NO_LAUNCH` at 0/240 and 0/400. V12 is complete excluded adaptive development at 28/28 and `SCREEN_NO_GO / NO_100E_LAUNCH`; V12b is a zero-optimizer-step train-only `STOP` audit, not a new model result. V13 is complete excluded adaptive development at 36/36 and `SCREEN_NO_GO / NO_100E_LAUNCH`; its smoke telemetry remains excluded and its conditional continuation is 0/72. V14 is prospective `READY_NOT_RUN`, not completed: its official screen is 0/40, every smoke value is excluded, and its continuation is unauthorized at 0/96. A confirmation successor must use new tasks/trajectories, freeze the host before opening them, and add separately frozen executed-return evaluation plus tuned recurrent/SSM/Transformer baselines.

**Recommendation.** The V3–V9 prospective studies all return negative pilot/final labels. V10-J fixes the end-to-end collapse problem but fails its prerequisite state-quality/convergence audit. V11b, V12, and V13 are complete and negative; the correct closeouts remain V11 `SCREEN_NO_GO / OFFICIAL_NO_LAUNCH` and V12/V13 `SCREEN_NO_GO / NO_100E_LAUNCH`. V13's 36-cell evidence is valuable diagnostic falsification, but it weakens rather than rescues the architecture/performance case: its safest effective variant removes correction, and KDIO plus the legal integrator remain stronger. V14 is an unrun adaptive proposal and cannot change that assessment. Do **not** submit the current version to the ICLR main track or describe V10–V14 as positive memory results. Retain the narrower workshop/diagnostic framing unless a separately frozen successor on new tasks clears representation, convergence, matched baselines, mechanism controls, and executed-return evaluation before any official launch.
