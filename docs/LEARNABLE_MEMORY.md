...
*Design and experiment record. Branch: `learnable-memory`. Implementations through `SharedActionShrinkageMemory` (HACSSM-v8/SAS-PC) live in `lewm/models/memory.py`; HACSSM-v9/LOIF is a design-only proposal, not code. Status as of 2026-06-29: every reported V1–V8 experiment is complete. V8 ran all 325 cells, 65,000 W&B epochs, and 325 rollout artifact/table/video bundles; its immutable pilot is `NO_GO` and final label is `PILOT_NO_GO_FINAL_DESCRIPTIVE`. Compact V8 ranks second by paired reduction (+6.20% versus SSM), but loses .23% to V7 shared-action, loses .96% to its retrained endpoint envelope, wins 0/5 task envelopes, and is not compact-noninferior under the frozen bootstrap (§7.10). LOIF-v9 (§2.8/§7.11) is the proposed response: learned ordered poles and evidence-conditioned gains/inverse-scale fusion under a single unweighted next-latent objective, with no fixed memory timescale or internal auxiliary. It has not been implemented or evaluated.*
...
| **HACSSM-v9 / LOIF** | proposed learned ordered stable poles + evidence-conditioned observation gains; no horizon auxiliary | proposed inverse-scale fusion of two posterior states | 34,563 proposed | design only: no implementation, run, or metric; removes hand-set memory timescales/objective weights |
...
### 2.8 HACSSM-v9 / LOIF proposal: learned ordered innovation filter

![HACSSM-v9 LOIF proposal](figures/fig_hacssm_v9_proposal.png)
*Figure 5e. Post-V8 design proposal, not an implementation or result. LOIF retains the two V8 mechanisms that remain large and stable—one shared action transition and a joint two-state read—but replaces fixed `τ={2,8}`, global shrinkage coefficients, and route logits by learned ordered stable poles, one causal evidence scale, Kalman-inspired gains, and inverse-scale fusion. The proposed controls make each change falsifiable. Section 7.11 records the design logic and proposed protocol.*

V8's failure is not simply “too few parameters.” It is already second by paired reduction and converged cleanly, while the expanded redundant control does not improve reliably. The problem is that one global static/dynamic compromise is wrong for different tasks and phases: static correction wins Ball-in-Cup/Cheetah and deep-blackout phases, dynamic correction wins Finger and other phases, and compact V8 loses .96% to the per-cell endpoint envelope. V9 should therefore adapt **evidence trust and the direct old-state coefficient**, not add another fixed auxiliary.
...
LOIF parameterizes retention rather than choosing a `τ` grid. The pole logits and the global evidence offset start at zero and are learned; this initialization is part of the architecture contract, not a swept timescale. With numerical `ε` only,
...
The nominated objective is one unweighted causal next-latent MSE averaged over the eligible visible targets of each episode (`first_post_loss_weight=0` in the legacy configuration). Every SSM/V7/V8 reference in the proposed grid must be retrained under that same objective; sealed historical checkpoints are evidence and provenance anchors, not objective-mismatched competitors. There is no memory teacher, target horizon set, counterfactual window, hidden-clean target, teacher momentum, auxiliary schedule, or memory-loss coefficient. Under MSE, `P/R` are operational positive scales—not calibrated posterior variances or probabilities—and “Kalman-inspired” describes the algebra only. A likelihood-calibrated model would be a different, separately frozen study and is outside the V9 grid.

This is not literally “hyperparameter-free”: `D`, the two-bank ceiling, one-step prediction, optimizer, data, sequence/gap distribution, numerical `ε`, and zero-logit initialization remain design choices. The narrower and testable claim is that V9 removes **hand-set memory timescales and memory-specific objective weights**. At `D=128,A=6`, with affine-free LN/RMSNorm, the proposed memory has `2D²+2AD+2D+3=34,563` trainable parameters—three fewer than compact V8—and streaming state `2D+2=258` floats. It remains sequential linear-time in sequence length. Tracking `log P` and using `logaddexp`/softmax avoids explicit inverse-`P` operations; `q=(1-α)(1+α)` avoids cancellation near one.

## 3. Why it is scalable

- **Linear time / memory.** Each bank/state is a diagonal recurrence: `O(L·K·D)` with `K` small (V5–V8 use `K=2`; V9 proposes the same ceiling). V6 adds bounded action rollouts; V7 adds bounded counterfactual windows and a training-only EMA copy; V8 removes those paths. Proposed V9 adds only two scalar positive-scale recurrences. None introduces `O(L²)` attention.
- **Parallelization is architecture-dependent.** A fixed or externally known affine recurrence can admit an associative scan. Proposed LOIF does **not** inherit that claim: its innovation-conditioned `R_t/K_t` depends on the current prior, so no associative scan has been established. The current code is sequential at `L=32`; there is no V9 code or long-sequence benchmark.
- **Explicit hierarchy with bounded state.** SMT-v1–v3 can be stacked by depth; V4 carries three fixed-rate belief levels, V5 carries two learned-rate levels with hard per-channel ordering, and V6–V8 use two fixed scalar-rate levels. V9 proposes two learned ordered poles plus scalar error scales, with no fixed memory horizon. Only V1–V8 are implemented; none has a parallel implementation or long-sequence scaling benchmark.
...
|---|---|---|---|
| **Mamba / S6** (Gu & Dao 2023) | learned input-dependent `Δ` | yes (learns timescale) | V1–V4 and V6–V8 fix rate bases; V5 learns global channel gains; proposed V9 learns ordered global poles plus input-dependent evidence, still not token-dependent `Δ` |
...
| **RetNet** (Sun et al. 2023) | fixed multi-scale decay per head | yes, through content-dependent Q/K/V (decay fixed) | V1–V8 expose action/gate/rate counterfactuals but use global routes; proposed V9 replaces the route with causal inverse-scale weights |
...
**Net positioning, revised after the experiments.** SMT occupies the fixed-decay-basis + learned-gating quadrant. SMT-v2 is functionally static; V3 mainly detects the explicit black token (§7.5). V4 establishes a genuine action-conditioned predict/correct path (§7.6). V5's learned spectrum, V6's dense consistency, and V7's counterfactual recovery fail their direct controls (§7.7–7.9). V8 removes those objective bundles and becomes the second paired-average design, but its compaction equivalence, tying advantage, endpoint envelope, task envelopes, and locked pilot all fail (§7.10). LOIF-v9 is a filtering hypothesis motivated by that phase/task heterogeneity, not a result (§7.11). The evidence supports causal filtering diagnostics—not architectural novelty, general hierarchy, or an ICLR performance claim.
...
3. **Selectivity and mechanism ablations — complete through V8.** Router mass matching, causal gate replacement, static/dynamic and old/new-recurrence controls, hard visibility, shifted masks, L0 cardinality, action/no-aux/single-level controls, memory-only action interventions, V5 rate/boundary controls, V6 objective controls, V7 action/shrinkage/recovery controls, and V8 action-tying/compaction/retrained-endpoint controls are complete (§7.5–7.10). A competitive Mamba-style learned-`Δ` baseline remains undone; V9 is only proposed.
4. **Router/update/action diagnostics — complete through V8.** SMT-v2 gates are nearly static (§5); V3 gates are black-sentinel selective (§7.5). V4–V8 repeatedly establish a causally used action path and joint read. V8 supplies the cleanest current estimates (+14.87% and +9.34%), while finding only +.16% for parameter-matched action tying and a −.96% loss to the retrained shrinkage endpoint envelope (§7.10). Proposed V9 would make evidence conditioning and the nominal direct-retention coefficient causal diagnostics (§7.11).
...
12. **Learned ordered innovation V9 — proposal only.** LOIF removes fixed memory timescales, shrinkage, route logits, and memory-specific auxiliary weights in favor of learned ordered poles and causal positive-scale updates. No code or experiment exists; direct controls and a held-out confirmation path are specified in §2.8/§7.11.
...
### 7.11 HACSSM-v9 / LOIF: post-V8 design and falsification proposal

**Status: brainstormed architecture only.** No V9 class, checkpoint, W&B run, or performance metric exists. Figure 5e and §2.8 describe the proposed filter; this section says what would have to be true before it should be called progress. Because V9 is selected after opening V8, another score on the same five tasks remains adaptive development and cannot be the paper's untouched confirmation.

The proposed modes isolate the hierarchy, adaptive evidence, evidence source, and fusion rather than treating every lower-scoring variant as support:

| proposed design | exact intervention | question |
|---|---|---|
...
The complete adaptive-development grid is therefore eight V9 modes plus **newly trained** SSM, V7 shared-action, compact V8, V8-dynamic, and V8-static references: `5 environments × 13 designs × 5 seeds = 325` cells. Seeds 0–2 form an immutable 195-cell pilot; seeds 3–4 are a mandatory 130-cell completion and cannot rescue a failed pilot. Every design uses the same unweighted visible-target MSE, so no sealed checkpoint is reused as an objective-mismatched competitor. A learned-variance NLL is deliberately outside this V9 protocol; adding it after opening MSE results would define another study. Reusing the feature/rollout inputs still requires V8's online W&B, fixed-rollout, clean-git, manifest, paired-relative, and no-pooled-raw-MSE contracts.

The proposed pilot/final screen is exact enough to falsify the design before implementation. A row applies separately to every reference named in that row; reductions are paired on `clean_mse_first_post` and lower is better.
...
The diagnostic log must include `α_f,α_s,q_f,q_s,log R_t,K_t^f,K_t^s,log P_t^f,log P_t^s,ω_t^f,ω_t^s,π_t^f,π_t^s` and the **nominal direct coefficient** `α_k(1-K_t^k)` by visible/transition/deep-blackout/recovery phase. It should report pole separation and boundary saturation without imposing a diversity loss, innovation/scale relationships, state/action perturbation or impulse-response influence, finite-value/saturation receipts, and batch-size-one streaming behavior. Hidden clean blackout latents remain evaluation-only. A direct control win/tie, noncausal reliability effect, collapsed poles, numerical saturation, or another deep-blackout regression is a scientific stop condition—not motivation for an unplanned V10 on the same grid.

For an ICLR claim, the better path is to implement LOIF, run only excluded engineering smokes on the current data, freeze it, and evaluate a separately held-out cohort: new rollout seeds; randomized gap positions and lengths; at least one unseen corruption family rather than the exact black sentinel; simulator-state prediction or a state probe; and executed task return/success with tuned recurrent/SSM/Transformer controls. V9's elegance is that the proposed hierarchy can be rejected cleanly: if learned poles and evidence-derived gains do not beat their direct controls, fixed/simple filtering remains the supported answer.
...
5. **The stronger common-target result is still bounded by simple controls and task heterogeneity.** The 100-epoch grid changes the winning architecture and is still improving late. Later V6–V8 studies converge, but no design wins every task. In the V8 ladder, static V6 wins two envelopes, dynamic V8, SSM, and V7-no-recovery win one each; compact V8 wins none and ranks 5th/3rd/3rd/7th/7th. V7 shared-action remains the best paired average (+6.43% versus SSM), only .22 percentage points above V8, but neither is untouched. Every mask still uses the same extreme black sentinel on reused validation trajectories, and no result is state decoding or return.
6. **The broad learned-selectivity/hierarchy thesis is still unsupported, despite real action mechanisms.** V3 and every V4 correction level separate one conspicuous black token. V4–V8 establish a real action path and useful joint read, but V4's slow state, V5's learned spectrum, V6/V7's self-supervised objectives, and V7's level-specific action/recovery terms fail. V8 removes the special objective yet remains 1.21% worse than SSM in deep blackout; its global learned shrinkage loses .96% to the per-cell retrained endpoint envelope. This supports a black-sentinel predict/correct diagnostic—not yet a generally superior content-selective or automatically sized hierarchy. LOIF-v9 is a falsifiable response, not evidence.
...
The fixed EMA bank is intentionally a simple diagonal state-space/RetNet special case; RetNet already combines fixed multi-scale decays with input-dependent Q/K/V content selection. SMT-v2's gates are empirically static, V3 mainly recovers freezing under explicit missingness, and V4–V8 are compact predict/correct recurrent filters. V8's compact no-auxiliary filter is effective but does not win its direct controls; proposed V9 overlaps established adaptive filtering/Kalman and selective-state-space ideas until evidence shows otherwise. Perfect sentinel detection, a useful action path, and shrinkage therefore remain diagnostic contributions; they do not create a defensible architectural “missing quadrant” without stronger tasks and outcomes. The related-work comparison must also include at least:
...
4. Make the diagnostic result central: static V2 gates, amplitude confounding, failed sparse frontier, V3's exact black-sentinel gate, V4–V8's causally necessary action path and joint read, positive-but-heterogeneous shrinkage, failed slow state/rate spectrum/self-supervised objectives/action specialization/recovery/compaction-equivalence, and the conditions where SSM/hold win. Remove “general input-conditioned selectivity,” “overall best,” and “automatic sizing” as achieved contributions.
...
7. Treat completed V7/V8 strictly as adaptive development evidence. If LOIF-v9 is implemented, use the present tasks only for excluded engineering smokes, freeze it, and evaluate unseen rollout seeds, gap distributions, and corruption families plus simulator-state prediction and executed return. Another V9 architecture search on the same five means is not confirmation.

**Recommendation.** The V3–V8 prospective studies all return negative pilot/final labels. V8 is the strongest nominated model versus SSM (+6.20%) and the second paired-average design, but it loses to V7 shared-action, fails noninferiority/equivalence/endpoint-envelope gates, and wins no task envelope. LOIF-v9 is a plausible clean experiment, not a reason to revise that evidence today. The expected main-track outcome remains rejection on task validity, baseline strength, and novelty/positioning. Do **not** submit the current version to the ICLR main track. Either (a) freeze one design and run untouched corruptions/seeds, state and executed-return outcomes, uncertainty, and tuned baselines before rebuilding around causal memory diagnostics, or (b) submit a narrower workshop paper centered on static V2, black-sentinel V3, action-sensitive V4–V8, and the repeated failure of hierarchical self-supervision.
