# Learnable memory for LeWorldModel: V1–V14 evidence and the proposed V15 predictive filtration

*Design and experiment record. Branch: `learnable-memory`. Implementations through KDIO-v11 live in `lewm/models/memory.py`; SIRO-v12, CF-HIRO-v13, and CF-EBO-v14 are isolated in `lewm/models/siro.py`, `lewm/models/cf_hiro.py`, and `lewm/models/cf_ebo.py` and integrated through the common LeWM host. V15 is a proposal only: no V15 source, checkpoint, run, or result exists. Status as of 2026-06-30: every reported V1–V9 experiment is complete. V9 ran all 325 cells, 65,000 W&B epochs, and 325 rollout artifact/table/video bundles; its immutable pilot is `NO_GO` and final label is `PILOT_NO_GO_FINAL_DESCRIPTIVE`. Full LOIF improves 2.55% over its freshly trained SSM, ranks 7/13, loses 4.15% to retrained compact V8 and 4.99% to the V8 endpoint envelope, and wins 0/5 task envelopes. Learned poles lose .67% to fixed poles and the two-bank model loses 2.22% to its single-bank control. Compact V8 is the strongest freshly retrained V9-grid design at +6.38% versus SSM; the earlier sealed V8 cohort instead ranked V7 shared-action first and compact V8 second, so the cohorts are reported separately (§7.10–7.11). **V10-J fixes causal representation collapse but fails its five-task state-quality/convergence screen, so its official grid remains unlaunched at 0/225 (§2.9/§7.12). V11's mandatory 32-cell development screen is negative, leaving the official V11 ledger at 0/240 pilot and 0/400 final (§2.10/§7.13). SIRO-v12 completes all 28 adaptive bundles but loses KDIO and its legal integrator, fails seven rank checks, collapses algebraically, and has negative held-fold action partial `R²` on every V12b task; its outcome is `SCREEN_NO_GO / NO_100E_LAUNCH` (§2.11/§7.14). CF-HIRO-v13 completes 36/36 and is also negative: full V13 scores `3.965301`, `nocorrect` scores `.816999`, and KDIO scores `.564411`; the conditional continuation remains 0/72 (§2.12/§7.15). CF-EBO-v14 has now completed its excluded adaptive screen at 40/40 cells, 1,200 W&B epoch rows, and 40 hashed rollout bundles. Full V14 scores `.896667`; unshrunk `norisk` is the best V14 mode at `.848872`; fresh V13-`nocorrect` scores `.791981`; and fresh KDIO is best overall at `.568321`. Artifact integrity and the registered robustness receipts pass, but representation, numerical, external, internal, action-mechanism, and convergence gates fail. The independently reproduced decision is `SCREEN_NO_GO`; `continue_to_100_epochs=false`, automatic launch is false, and the conditional continuation remains 0/96 (§2.13/§7.16). V15 Cross-View Predictive Filtration is recorded only as `PROPOSED_NOT_IMPLEMENTED` (§2.14).***
...
**Empirical verdict.** SMT-v2 does not support the selection hypothesis: its gates are almost static and can be replaced by calibration means without hurting the saved models (§5). L0 routing finds either a dense model or a quality-destroying closed/static subset, and the strongest OC-SMT result uses all 28 banks (§9). SMT-v3 fixes the erasing write rule and learns a causally important black-sentinel gate (§7.5). HACSM-v4 then fixes action blindness: swapping only memory-path blackout actions raises first-post MSE by 12.46%, and V4 beats V3 by 12.49% across paired cells (§7.6). V5's learned channel spectrum and boundary auxiliary regress (§7.7); V6's dense consistency adds only .39% and loses to static correction (§7.8); V7's recovery objective adds only .30% and loses its shared-action/no-recovery controls (§7.9). V8 cleanly removes those special objectives and confirms the durable mechanisms—action transport +14.87%, joint two-state read +9.34%—but the compact/shared-action novelty does not clear its controls. Its learned global shrinkage is interior and beats each endpoint on average, yet loses .96% to selecting the better retrained endpoint per cell and is 1.21% worse than SSM in deep blackout (§7.10). V9 confirms that causal evidence conditioning and action transport are active—full LOIF beats global resistance by 30.56% and no-action by 8.29%—but rejects its intended hierarchy: fixed poles beat learned poles, one slow bank beats two banks, the sample-specific resistance interventions are sub-.5%, and full LOIF remains behind V7/V8 (§7.11). V10's causal host repair preserves rank, but its rotation-only prior loses to a simple action integrator on all five tasks and its original cross-coordinate probe is partly mis-specified (§7.12.6). V11's mechanics-shaped hierarchy fails its mandatory development screen, and SIRO-v12's synchronized self-identification loses KDIO and its legal integrator, fails seven rank checks, collapses algebraically, and has negative held-fold action partial `R²` on all four tasks. CF-HIRO-v13 fixes nonnormality and the ambiguous stream decomposition, but its fixed Riccati correction amplifies off-fit innovations; `nocorrect` is its strongest mode. CF-EBO-v14 then completes the bounded, cross-fold-calibrated repair and returns another negative result. The safety layer behaves in the narrow registered sense—Gaussian radial suppression and every correction-energy bound pass—but full `.896667` ranks only fifth of ten, loses fresh KDIO `.568321`, fresh V13-`nocorrect` `.791981`, its own `nocorrect` `.890761`, and its checkpoint-matched integrator `.507053`. `norisk` is the best V14 mode at `.848872`, so the empirical-Bayes shrinkage does not improve this cohort. Final deployed action reliability is exactly zero on all four tasks, action suffix advantage is zero and pair accuracy is chance, while correction reliability is `.9989–.9995` even though full loses `nocorrect` on all four. Encoder effective rank fails on Cartpole, Fish, and Walker; the mixed-precision runtime direct-sum check fails in all 18 corresponding V14-mode cells; and convergence fails badly. V14 is therefore `SCREEN_NO_GO / NO_100E_LAUNCH`, not an overall best performer or an ICLR-ready method (§7.16). V15 is a self-supervised predictive-filtration hypothesis motivated by these failures, not evidence (§2.14).

## 2. The architecture

![Architecture map of tested memory designs](figures/fig_memory_versions_arch.png)
*Figure 1. Consolidated map of the V1→V14 evidence and the proposed V15 successor. V14 retains V13's normal coordinate, adds machine-support observable-energy coordinates, opposite-fold risk calibration, OAS whitening, a unit correction cap, and radial suppression. Its completed 40/40 adaptive screen is `SCREEN_NO_GO`: `norisk` is the best V14 mode, KDIO is best overall, and the conditional continuation remains 0/96. The V15 Cross-View Predictive Filtration panel is `PROPOSED_NOT_IMPLEMENTED`; it is an architecture hypothesis, not a result.*
...
| **CVPF-v15 proposal** | nested future-predictive conditional-expectation increments for initial anchor, executed actions, and observed innovations; OAS-whitened cross-view canonical modes; per-mode opposite-fold evidence; implicit differentiation through identification | direct future-output predictive state; no reconstructed hidden state, dense complement projector, or hidden-state pseudoinverse | no implementation or parameter count; intended depth/rank comes from cross-fold predictive support rather than a task-set threshold | **`PROPOSED_NOT_IMPLEMENTED`**: architecture and falsifiable controls only; 0 runs, 0 checkpoints, no performance or novelty result |
...
![CF-EBO-v14 architecture, completed screen, causal closeout, and proposed V15 successor](figures/fig_cf_ebo_v14_arch.png)
*Figure 5j. Completed CF-EBO-v14 architecture and evidence. A pooled V13 real-normal realization defines the source coordinate, an infinite-horizon observability Gramian converts its machine-active subspace into energy coordinates, and inactive directions are exactly zero-padded in the fixed schema. Even/odd train episodes calibrate action and correction by opposite-fold positive-part empirical-Bayes risk; OAS-whitened innovations pass through a unit-spectral correction map and rank-derived radial gate. All six V14 modes and four fresh baselines completed at 40/40 artifact-valid cells. The result is `SCREEN_NO_GO`: full `.896667`, best V14 `norisk` `.848872`, V13-`nocorrect` `.791981`, and best-overall KDIO `.568321`; the conditional continuation remains 0/96. The bottom panel records V15 CVPF only as `PROPOSED_NOT_IMPLEMENTED`.*
...
### 2.14 CVPF-v15: Cross-View Predictive Filtration (`PROPOSED_NOT_IMPLEMENTED`)

V15 is a design response to V14's completed failures, not an implemented version. There is no V15 module, optimizer state, screen manifest, W&B run, rollout, checkpoint, or empirical value anywhere in this document. Its purpose is to state the next falsifiable hypothesis before code or data are opened.

#### 2.14.1 A hierarchy defined by information, not named state slots

Let `F_t` be the OAS-whitened block of available future clean embeddings after time `t`. Instead of reconstructing an unobserved state through `H†`, define three nested information sets:

```text
H0_t = sigma(z_0)
Ha_t = sigma(z_0, a_<t)
Ho_t = sigma(z_0, a_<t, z^observed_<=t)
```

and their `L2` predictive projections:

```text
p_anchor,t = E[F_t | H0_t]
p_action,t = E[F_t | Ha_t] - E[F_t | H0_t]
p_obs,t    = E[F_t | Ho_t] - E[F_t | Ha_t]
p_t        = p_anchor,t + p_action,t + p_obs,t.
```

These are conditional-expectation, or martingale-difference, increments. In the population linear-prediction geometry, each new increment is orthogonal to the earlier information space. The roles therefore have an operational meaning: the anchor is future information already present at the initial observation; the action component is additional future information carried predictively by the observed action history; and the observation component is additional future information carried by later observations. The action increment is **not** automatically a causal action effect: that interpretation would additionally require a stated sequential-ignorability/randomization and overlap contract for the behavior policy. A role may become exactly zero when it adds no reproducible predictive value. Unlike V12's `r+u` bookkeeping split, the hierarchy cannot be renamed away without also removing a conditional-information increment.

```text
 clean future block F_t
          ▲
          │  opposite-fold future-output loss
          │
 [ p_anchor ] ── + ── [ p_action ] ── + ── [ p_obs ] ──► predictor
      ▲                    ▲                   ▲
      │ E[F|z0]            │ action-history      │ observed-innovation
      │                    │ predictive increment│ predictive increment
      z0                 a_0,...,a_{t-1}      z^observed_1,...,z^observed_t
                         nested predictive filtration
```

#### 2.14.2 Cross-view canonical modes and end-to-end identification

The proposed finite-sample layer whitens past/source and clean-future blocks with closed-form OAS estimates and computes cross-view canonical modes. For mode `j`, let `rho_j in [0,1]` be its fitted canonical correlation and let `w_j in [0,1]` be the symmetric positive-part opposite-fold predictive-risk weight, using the same two-direction evidence principle as V14 but scoring that single future-output mode. The deployed directional coefficient is explicitly `g_j=rho_j w_j`: `rho_j` measures cross-view predictive strength, while `w_j` measures held-fold retention. Effective support is `{j:w_j>0}`; unsupported modes receive exact zero without V14's binary FP64 machine-rank cutoff. Executed actions are residualized jointly against the earlier information set, so the action certificate measures incremental future-output prediction rather than hidden-coordinate reconstruction quality. Observation innovations are mapped directly to future-output residuals, so correction calibration, deployment, and evaluation share one predictive target.

Compression does **not** by itself preserve the suffix shift. V15 must therefore either retain a shift-closed future block or identify a projected shift operator `S` from consecutive canonical predictive coefficients and audit its opposite-fold closure and recursive suffix error. The proposed default is the latter: initialization, action history, and observation innovation update canonical future-prediction coefficients; a train-only minimum-norm projected solve fits `S`; and any mode without positive held-fold recursive value receives `w_j=0`. The predictor reads the current/next-output block directly. There is no dense output-complement projector, hidden-state pseudoinverse, reconstructed all-futures hidden-state target, scalar global correction confidence, or norm-only radial decision.

Most importantly, the opposite-fold future-prediction loss would differentiate through the covariance solves and canonical decomposition, using implicit differentiation or an equivalent differentiable symmetric eigensolver. The encoder would therefore be optimized to preserve reproducible action- and observation-predictive directions. V14 instead differentiates through an installed recurrence while treating the identification response to encoder changes as detached.

#### 2.14.3 What could be novel, and what is already known

Predictive-state representations, canonical-correlation/subspace identification, Doob/martingale decompositions, OAS covariance shrinkage, cross-validation, and differentiable linear-algebra layers are established. V15 cannot claim any of them individually. The possible contribution is narrower: a **cross-view predictive filtration layer** whose anchor/action/observation hierarchy is defined by nested conditional predictive information, whose canonical basis strength `rho_j` is separated from held-fold support `w_j`, whose projected shift is explicitly identified and closure-audited, and whose identification is differentiated end to end inside LeWM. A useful theorem target would show gauge invariance, orthogonality of the population increments, and monotone decomposition of future predictive energy.

This is intended to remove task-tuned rank, ridge, pole, correction-gain, horizon-weight, and auxiliary-loss coefficients. It is not literally hyperparameter-free: the clean-future block, fold construction, OAS target, encoder width, and numerical eigensolver remain structural choices. Using all available lags and data-derived continuous mode weights avoids selecting those quantities on the four opened development tasks.

#### 2.14.4 Required falsification before any launch

An implementation must first pass mixed-precision output-equivalence, streaming, prefix-causality, gauge-alignment, projected-shift closure/recursive-suffix, rank, and implicit-gradient checks. Direct controls must remove each information increment, stop gradients through identification, replace `w_j` by one while retaining `rho_j`, remove `rho_j` while retaining held-fold support, and replace the filtration with a same-width ordinary recurrent state. Any causal action claim additionally requires a frozen behavior-policy randomization/overlap audit. The frozen comparison must include V14-`norisk`, V14-`nocorrect`, V13-`nocorrect`, KDIO-v11, SSM, compact V8, the checkpoint-matched legal integrator, and a modern sequence baseline. Because V10–V14 were adaptively designed on the same four caches, any performance claim requires unopened tasks or trajectories and separately frozen executed-return/success evaluation. Until those conditions are met, the only valid V15 label is **`PROPOSED_NOT_IMPLEMENTED`**.
...
- **A hierarchy is a falsified-or-supported mechanism, not an assumption.** SMT-v1–v3 can be stacked by depth; V4 carries three belief levels; V5–V8 carry two; and V9 carries two learned poles, but V9's one-bank control wins. V10 therefore uses one state. V11 reintroduces exactly two coupled roles—configuration and velocity—and its development controls reject the full composition. V12 nominally stores three causal roles, but full SIRO's identity read and shared `A` make `r+u` an exact sufficient dynamic state. V13's normal blocks have data-derived radii/phases and its complement is algebraically orthogonal to the dynamic read, but the completed screen does not support a useful modal hierarchy. V14 also fails to establish one: its final machine-active supports are full `24/120/24/144`, while encoder effective ranks are only `2.01/12.18/54.80/1.11`, and its action path is closed on all four tasks. V15 therefore proposes conditional-information increments whose unsupported levels may vanish; that hierarchy remains unimplemented.
- **Streaming counts are not training peaks.** V1–V9 need `K·D` state (plus V9's two scales), V10 needs `D`, V11 needs `2D`, and the implemented V12 API stores `3D`; full V12 is algebraically reducible to `2D` (`c` plus `r+u`). V13 and V14 store a fixed `n+D` streaming schema and no covariance, even when V14's active support rank is below `n`. Their training implementations retain encoder activations, scans, paired detached views, all-lag Hankels, reconstructed states, and dense FP64 fit tensors. The completed V14 peak-VRAM and timing receipts therefore describe training/fitting, not its smaller streaming state.
...
**Net positioning, revised after the experiments.** SMT occupies the fixed-decay-basis + learned-gating quadrant. SMT-v2 is functionally static; V3 mainly detects the explicit black token (§7.5). V4 establishes a genuine action-conditioned predict/correct path (§7.6). V5's learned spectrum, V6's dense consistency, and V7's counterfactual recovery fail their direct controls (§7.7–7.9). V8 removes those objective bundles and becomes a leading paired-average design, but its compaction equivalence, tying advantage, endpoint envelope, task envelopes, and locked pilot all fail (§7.10). V9's filtering algebra produces active evidence and action paths, yet its learned poles and two-bank hierarchy lose direct controls and full LOIF ranks seventh in its retrained grid (§7.11). V10-J fixes host collapse but fails its prerequisite audit (§2.9/§7.12). V11's kick–drift/OAS composition then fails its completed 32-cell launch screen, and V12's completed identified-observer screen loses KDIO/integrator, fails rank, collapses algebraically, and lacks held-fold action signal (§2.10–2.11/§7.13–7.14). V13's split-agreement predictive realization and real-normal dynamics pass their numerical checks, but the completed screen rejects the composed observer. V14's 40-cell screen then shows that bounded radial correction repairs a narrow safety property without producing a competitive or action-sensitive memory: full ranks fifth of ten, `norisk` is the strongest V14 mode, correction loses the exact `nocorrect` control, action reliability is zero on all tasks, representation/numerical/convergence gates fail, and KDIO remains best overall. V14 is `SCREEN_NO_GO / NO_100E_LAUNCH`; it is a useful negative diagnostic composition, not an ICLR method. V15's predictive filtration is a proposed attempt to align hierarchy, mode selection, and end-to-end identification in future-output space; at zero implementations and zero runs it has no empirical positioning or novelty evidence (§2.14/§7.16).
...
18. **Cross-View Predictive Filtration CVPF-v15 — `PROPOSED_NOT_IMPLEMENTED`.** The proposed hierarchy is the sequence of incremental clean-future predictions contributed by the initial observation, executed actions, and observed innovations. OAS-whitened cross-view canonical modes, per-mode opposite-fold evidence, and implicit gradients are intended to replace V14's machine-rank hidden coordinate, scalar reliabilities, and detached identification. No code, run, checkpoint, W&B row, or performance result exists (§2.14).
...
The result weakens rather than rescues the current ICLR case. Full V14 is behind KDIO, V13-`nocorrect`, its own `nocorrect`, `norisk`, and the legal integrator; three representation ranks and 18 runtime reconstruction cells fail; and no 100-epoch continuation is authorized. V15's proposed conditional-information hierarchy could create a clearer architectural and theoretical object, but PSRs, CCA, martingale decompositions, and differentiable solvers are established ingredients. Only a working implementation, explicit novelty theorem or analysis, unopened confirmation tasks, competitive modern baselines, and executed returns could change the submission recommendation. Until then, V15 is `PROPOSED_NOT_IMPLEMENTED`, and neither V14 nor V15 is an ICLR-ready positive method result.
...
The fixed EMA bank is intentionally a simple diagonal state-space/RetNet special case; RetNet already combines fixed multi-scale decays with input-dependent Q/K/V content selection. SMT-v2's gates are empirically static, V3 mainly recovers freezing under explicit missingness, and V4–V9 are compact predict/correct recurrent filters. V8's compact no-auxiliary filter is effective but does not win its direct controls; completed V9 overlaps established adaptive filtering/Kalman and selective-state-space ideas and then loses its learned-pole/two-bank controls. V10's exact action-conditioned rotations are more structurally distinctive, but unitary RNNs, Homomorphism Autoencoders, seq-JEPA, and FloWM already cover norm preservation, learned group actions, sequential JEPA aggregation, and structured partially observed dynamics. V11's potentially useful combination is narrower: learned amplitude times a Stiefel action direction inside an exactly invertible kick–drift prior, ordered clean-OAS correction, and detached same-source executed/deranged displacement ranking beside a live suffix loss. Orthogonal parameterizations, reversible mechanics priors, robust/Kalman filtering, and contrastive ranking each exist separately. Its completed screen supplies no performance-based novelty claim. V12's proposed contribution is also a composition rather than a new primitive, and its exact collapse plus negative results eliminate a hierarchy/performance claim. V13 is likewise a composition: Ho–Kalman/ERA and subspace methods establish Hankel realization, DMDc establishes control-input dynamics, PSRs establish observable predictive state, and OAS/DARE/Kalman filtering establish its correction machinery. The completed screen shows that positive-part fold agreement, normal projection, and a canonical output-complement anchor are not sufficient for an effective observer. V14 adds an observability-Gramian coordinate, cross-fold empirical-Bayes shrinkage, spectral clipping, and radial influence, all established ideas. Its completed 40-cell screen supplies negative rather than performance-based novelty evidence: `norisk` and `nocorrect` beat full, action closes on every task, and detached pooled refits remain neither fully cross-fitted nor differentiable identification. V15's proposed conditional predictive filtration could be a more coherent object, but PSRs, CCA, Doob decompositions, cross-validation, and implicit layers are also established. A novelty claim would require a precise new formulation or theorem plus positive unopened-task evidence. A future comparison would still have to include:
