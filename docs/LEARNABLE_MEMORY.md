...
- **V13 increases detached dense identification work.** Every refit encodes both train views, accumulates all-lag fold moments, decomposes a block Hankel matrix, performs a real-Schur normal projection, refits `B` across all lags, reconstructs clean predictive states, estimates OAS covariances, and solves a DARE. Its zero optimizer parameters and covariance-free online state do not imply cheap fitting. The completed screen records fit time, state width, operator payload size, epoch time, and peak VRAM, but does not support an efficiency advantage.
...
#### 7.15.6 Untested successor direction—not a V14 result

The evidence suggests a narrower successor, but none is implemented, registered, or automatically launched. Replace coefficient-agreement shrinkage by **positive held-fold predictive-risk shrinkage**: retain an action mode only to the degree that fitting it on one train fold improves future-output prediction on the other, symmetrized across folds. Replace the unconditional OAS/DARE gain by a **self-supervised robust innovation evidence mixture** that continuously interpolates between corrected and open-loop predictions; fit its reliability from train-only paired and cross-fit residuals so heavy-tailed/off-fit innovations can drive the gain toward zero without reward, state labels, corruption identity, or hand-selected task thresholds. Make the complement path explicitly rank-aware, gating it off when the fitted output map already spans the encoder coordinate rather than treating a near-zero complement as a hierarchy.

That proposal directly targets V13's three empirical failures—agreement without held-fold action value, Gaussian correction amplification, and Walker's absent complement—but it remains a composition of known predictive-state, robust-filtering, and rank-selection ideas. A materially stronger end-to-end LeWM novelty claim would additionally require differentiating through, or using an implicit gradient for, the fitted realization/filter so encoder features are shaped by predictive-state identification rather than merely refitted after each epoch. Such a change would need a new frozen protocol, unopened confirmation data, competitive returns, and explicit controls. There is **no automatic V14 launch** and no successor result in this document.
