...
*Figure 1. Consolidated map of the V1→V12 evolution, architecture-changing controls, every completed V5–V9 mechanism variant, all five V10 modes, the registered 16-design V11 grid, and SIRO-v12. V11 has three external references, full scaled-ASR KDIO, and twelve explicit KDIO controls. V12 replaces its learned mechanics with a conserved anchor and separately identified residual/action streams. This is an architecture/protocol map: V10 remains 0/225 official cells; V11 is `SCREEN_NO_GO / OFFICIAL_NO_LAUNCH` at 0/240 pilot and 0/400 final; V12 completed its excluded 28/28 adaptive screen and is `SCREEN_NO_GO / NO_100E_LAUNCH`.*
...
- **A hierarchy is now a falsified-or-supported mechanism, not an assumption.** SMT-v1–v3 can be stacked by depth; V4 carries three belief levels; V5–V8 carry two; and V9 carries two learned poles, but V9's one-bank control wins. V10 therefore uses one state. V11 reintroduces exactly two coupled roles—configuration and velocity—and its development controls reject the full composition. V12 nominally stores three causal roles, but full SIRO's identity read and shared `A` make `r+u` an exact sufficient dynamic state. Its `(kappa,tau)` reachability-age spectrum is diagnostic and does not allocate or select a hierarchy.
- **Streaming counts are not measured training peaks.** V1–V9 need `K·D` state (plus V9's two scales), V10 needs `D`, V11 needs `2D`, and the implemented V12 API stores `3D`; full V12 is algebraically reducible to `2D` (`c` plus `r+u`). Training implementations retain scans and encoder activations; full V11 additionally retains the suffix triangle, while V12 additionally retains detached fit views and dense FP64 statistics/operators. The experiment records peak VRAM, epoch time, and fit time before making an efficiency claim.
...
**Cautious V13 prospect—CF-HIRO, unrun.** An isolated **cross-fold-agreement** prototype explores an all-lag action-to-future Hankel realization, continuous even/odd fold-agreement shrinkage, real-Schur stable coordinates, paired-view OAS process/measurement covariances, and a Joseph-form Riccati observer. The acronym means cross-fold agreement, not cross-fitting: the realization learns one pooled even/odd SVD basis and the covariance fit uses all episodes, so no estimate is out-of-fold. This would replace SIRO's arbitrary additive stream labels with a predictive-state realization whose coordinates are defined by action-to-future moments, and would address V12b's nonnormality/exposure failures through Schur deployment and recursive covariance correction. It still assumes sequentially exogenous randomized actions; its balanced Hankel split, full numerical order, fold-shrink rule, stability projection, and covariance model are structural choices rather than learned truths. CF-HIRO has only synthetic correctness tests: it is not integrated into the LeWM trainer, has no DMC/W&B/rollout result, is not V13 evidence, and is not claimed as novel or superior. Any V13 study must be prospectively frozen and must first beat no-action, simple integrator, tuned recurrent/SSM/Transformer baselines, and the current compact-V8/KDIO controls on new trajectories and executed-return outcomes.
...
7. Treat completed V7–V12 strictly according to scope. V7–V9 have immutable negative labels. V10-J is an adaptive representation-validity audit with a `NO LAUNCH` decision; V11b is an excluded adaptive `SCREEN_NO_GO`, not a pilot, and its official grid is `OFFICIAL_NO_LAUNCH` at 0/240 and 0/400. V12 is complete excluded adaptive development at 28/28 and `SCREEN_NO_GO / NO_100E_LAUNCH`; V12b is a zero-optimizer-step train-only `STOP` audit, not a new model result. A confirmation successor must use new tasks/trajectories, freeze the host before opening them, and add separately frozen executed-return evaluation plus tuned recurrent/SSM/Transformer baselines.

**Recommendation.** The V3–V9 prospective studies all return negative pilot/final labels. Full V9 is only +2.55% versus SSM, ranks 7/13, loses to V7/V8/fixed-`α`/single-bank controls, and wins no task envelope; compact V8 leads its freshly retrained grid at +6.38%. V10-J fixes the end-to-end collapse problem but fails its prerequisite state-quality/convergence audit. V11b is complete and negative: its default loses the legal integrator, raw-difference ranking is only marginally better, mechanism gates do not jointly clear, and 32/32 curves worsen late. V12 is likewise complete and negative: all 28 bundles are valid, but full SIRO loses KDIO and the integrator, fails representation/convergence checks, collapses algebraically to one dynamic state, and V12b returns `STOP`. The correct closeouts are V11 `SCREEN_NO_GO / OFFICIAL_NO_LAUNCH` and V12 `SCREEN_NO_GO / NO_100E_LAUNCH`. Do **not** submit the current version to the ICLR main track or describe V10–V12 as positive memory results. Retain the narrower workshop/diagnostic framing unless a separately frozen successor on new tasks clears representation, convergence, matched baselines, mechanism controls, and executed-return evaluation before any official launch.

## References

Gu & Dao. *Mamba: Linear-Time Sequence Modeling with Selective State Spaces.* 2023 (arXiv:2312.00752). · Ma et al. *Mega: Moving Average Equipped Gated Attention.* 2022 (arXiv:2209.10655). · Qin et al. *HGRN2: Gated Linear RNNs with State Expansion.* 2024 (arXiv:2404.07904). · Sun et al. *Retentive Network (RetNet).* 2023 (arXiv:2307.08621). · Becker et al. *Recurrent Kalman Networks: Factorized Inference in High-Dimensional Deep Feature Spaces.* ICML 2019 (arXiv:1905.07357). · Hefny et al. *Recurrent Predictive State Policy Networks.* ICML 2018. · Wisdom et al. *Full-Capacity Unitary Recurrent Neural Networks.* NeurIPS 2016 (arXiv:1611.00035). · Keurti et al. *Homomorphism Autoencoder.* 2022. · Ghaemi et al. *seq-JEPA.* NeurIPS 2025. · Lillemark et al. *Flow Equivariant World Modeling for Partially Observed Dynamic Environments.* 2025. · Behrouz et al. *Titans: Learning to Memorize at Test Time.* 2025 (arXiv:2501.00663). · *Instance-Conditional Timescales of Decay for Non-Stationary Learning.* (arXiv:2212.05908). · Yang et al. *Gated Linear Attention.* 2023 (arXiv:2312.06635). · Companion paper: `docs/ICLR.md` (§5.4 one global learned scalar decay does not self-tune in this setup; §5.11 fixed K-bank beats the tested learned memories under a matched budget).
