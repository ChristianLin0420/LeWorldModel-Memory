# Learnable memory for LeWorldModel: SMT-v1–v3 and HACSM/HACSSM/ORBIT/KDIO/SIRO/CF-HIRO-v4–v13

*Design and experiment record. Branch: `learnable-memory`. Implementations through KDIO-v11 live in `lewm/models/memory.py`; SIRO-v12 and CF-HIRO-v13 are isolated in `lewm/models/siro.py` and `lewm/models/cf_hiro.py` and integrated through the common LeWM host. Status as of 2026-06-30: every reported V1–V9 experiment is complete. V9 ran all 325 cells, 65,000 W&B epochs, and 325 rollout artifact/table/video bundles; its immutable pilot is `NO_GO` and final label is `PILOT_NO_GO_FINAL_DESCRIPTIVE`. Full LOIF improves 2.55% over its freshly trained SSM, ranks 7/13, loses 4.15% to retrained compact V8 and 4.99% to the V8 endpoint envelope, and wins 0/5 task envelopes. Learned poles lose .67% to fixed poles and the two-bank model loses 2.22% to its single-bank control. Compact V8 is the strongest freshly retrained V9-grid design at +6.38% versus SSM; the earlier sealed V8 cohort instead ranked V7 shared-action first and compact V8 second, so the cohorts are reported separately (§7.10–7.11). **V10-J fixes causal representation collapse but fails its five-task state-quality/convergence screen, so its official grid remains unlaunched at 0/225 (§2.9/§7.12). V11 is the adaptive kick–drift/OAS response. Its mandatory 32-cell V11b development screen is complete and negative, leaving the official V11 ledger at 0/240 pilot and 0/400 final (§2.10/§7.13). SIRO-v12 then completes all 28 adaptive W&B/rollout bundles but loses retrained KDIO-v11 and its legal integrator, fails seven rank checks, collapses algebraically to one dynamic state, and has negative held-fold action partial `R²` on every V12b task; its outcome is `SCREEN_NO_GO / NO_100E_LAUNCH` (§2.11/§7.14). CF-HIRO-v13 is now implemented and protocol-frozen as the normal predictive-state successor, but its status is strictly `READY_NOT_RUN`: the 30-epoch screen is 0/36 and its conditional 100-epoch continuation is 0/72. Four full-size one-epoch no-W&B smokes establish only executable preflight—not performance, W&B, hierarchy, novelty, or ICLR evidence (§2.12/§7.15).***
...
Only if every screen gate passes, retain `{full,noaction,noshrink,SSM,compact V8,KDIO-v11}` and run `6 designs × 4 tasks × fresh seeds {13002,13003,13004} × 100 epochs = 72` cells. `fullanchor`, `triangular`, and `nocorrect` are screen-only diagnostic controls. Before launch the ledgers are exactly **0/36 and 0/72**, with no V13 W&B link or result.

The nearest primitives are established: Ho–Kalman/eigensystem realization and subspace identification define predictive coordinates from impulse moments; DMD with control estimates action-conditioned linear dynamics; predictive-state representations use observable future statistics; and OAS, DARE, and Kalman correction are classical. CF-HIRO's prospective distinction is the composition of positive-part split agreement, a normal all-lag realization, canonical output-complement anchoring, and fixed-gain correction inside an end-to-end LeWM loop. That is not a fundamental-new-primitive claim and is not yet an empirical contribution. It is also not hyperparameter-free: removing tuned loss weights and selected horizons does not remove the structural choices listed above.
...
- **V13 increases detached dense identification work.** Every prospective refit encodes both train views, accumulates all-lag fold moments, decomposes a block Hankel matrix, performs a real-Schur normal projection, refits `B` across all lags, reconstructs clean predictive states, estimates OAS covariances, and solves a DARE. Its zero optimizer parameters and covariance-free online state do not imply cheap fitting. Screen receipts must report fit time, state width, operator payload size, epoch time, and peak VRAM before any efficiency claim.
...
- **A hierarchy is now a falsified-or-supported mechanism, not an assumption.** SMT-v1–v3 can be stacked by depth; V4 carries three belief levels; V5–V8 carry two; and V9 carries two learned poles, but V9's one-bank control wins. V10 therefore uses one state. V11 reintroduces exactly two coupled roles—configuration and velocity—and its development controls reject the full composition. V12 nominally stores three causal roles, but full SIRO's identity read and shared `A` make `r+u` an exact sufficient dynamic state. V13's normal blocks have data-derived radii/phases and its complement is algebraically orthogonal to the dynamic read, but this is only a possible modal hierarchy. It is unsupported if agreement collapses, if modes do not survive controls, or if `C` has full row rank and the complement vanishes.
- **Streaming counts are not measured training peaks.** V1–V9 need `K·D` state (plus V9's two scales), V10 needs `D`, V11 needs `2D`, and the implemented V12 API stores `3D`; full V12 is algebraically reducible to `2D` (`c` plus `r+u`). V13 stores `n+D` streaming floats and no covariance, but its training implementation retains encoder activations, scans, paired detached views, all-lag Hankels, reconstructed states, and dense FP64 fit tensors. The experiment records peak VRAM, epoch time, fit time, and checkpoint payload before making an efficiency claim.
...
| **Ho–Kalman / ERA / DMDc / subspace identification** | impulse-response Hankel realization and action-conditioned linear dynamics | action-observed | establishes V13's all-lag realization, modal state, and control-input fit as known primitives; the prospective distinction can only be their split-agreement/normal/direct-sum composition inside LeWM, not predictive realization itself |
...
16. **Split-agreement normal predictive observer CF-HIRO-v13 — `READY_NOT_RUN`.** Core, trainer, integration, and full-size four-task smoke preflight pass, including exact streaming and same-schema controls. These checks used no online W&B and are not results. The frozen seed-`13001` screen remains 0/36; the conditional fresh-seed 100-epoch continuation remains 0/72 and may launch only after every conjunctive gate passes (§2.12/§7.15).
...
### 7.15 CF-HIRO-v13 prospective ledger — `READY_NOT_RUN`

The implementation described in §2.12 is integrated into the causal LeWM host and has synthetic, registry, trainer-objective, serialization, and end-to-end smoke coverage. That establishes readiness only. No registered screen cell has run, no V13 W&B run exists, and no value below is eligible for model comparison.

| ledger | designs | tasks | optimizer seeds | epochs | required cells | completed result cells | status |
|---|---:|---:|---:|---:|---:|---:|---|
| frozen adaptive screen | 9 | 4 | `{13001}` | 30 | 36 | **0** | **`READY_NOT_RUN`** |
| conditional continuation | 6 | 4 | `{13002,13003,13004}` | 100 | 72 | **0** | **not launched; requires every screen gate** |

The screen contains all six CF-HIRO modes plus freshly trained SSM, compact V8, and raw-difference KDIO-v11. The conditional wave retains only full, `noaction`, `noshrink`, SSM, compact V8, and KDIO. Four persistent workers are assigned to GPU IDs `0,1,2,3`. The runner refuses evidence launch unless the worktree is clean and `HEAD` equals its local upstream, and records the branch/commit/upstream/pushed state. A cell is complete only with online W&B epoch history, checkpoint, metrics, full fit/operator payload, rollout table/video/NPZ, SHA-256, and finished run receipt.

The screen analyzer must apply one conjunction, not pick a favorable subset after opening results:

1. all `36/36` artifacts and protocol hashes validate;
2. full V13 passes on 4/4 tasks with encoder mean channel variance at least `1e-5`, effective rank at least `16`, and singleton/prefix causal discrepancies at most `1e-5`; all six V13 modes have the exact registered fit count/schema/mode receipts, streaming/projector/reconstruction/orthogonality errors at most `1e-5`, steady Riccati relative residual at most `1e-6`, the registered spectral/operator bound, and exact no-action/no-correction receipts where applicable;
3. full V13 reduces equal-task held-out prior NMSE by at least 5% versus `noaction`, SSM, compact V8, KDIO-v11, and the checkpoint-specific legal initial-frame integrator, and by at least 2% versus `fullanchor`, `triangular`, `noshrink`, and `nocorrect`, with at least 3/4 task wins against every reference;
4. the train-only action map fit on even episodes has positive action-predictive `R²` on odd episodes and the odd-fit map has positive `R²` on even episodes for at least 3/4 tasks;
5. executed actions have positive open-loop suffix advantage and pair accuracy above `.5` on the same tasks;
6. complement-anchor RMS and dynamic-initial RMS are each greater than `1e-8` on all four full-V13 tasks, otherwise the direct-sum hierarchy is absent; and
7. every full task has nonnegative signed late predictive change, the maximum absolute full-task change is below 5%, and the median absolute change across all 36 cells is below 3%.

Failure of any item fixes the screen outcome to `SCREEN_NO_GO / NO_100E_LAUNCH`. Passing all items permits the 72-cell continuation but does not itself establish ICLR novelty or confirmation on unopened tasks.

#### 7.15.1 Full-size implementation preflight (excluded from results)
...
14. V13 currently contributes **no empirical evidence**. CF-HIRO's implementation and full-size no-W&B smokes are preflight; its 30-epoch ledger is 0/36 and conditional 100-epoch ledger is 0/72. `READY_NOT_RUN` must not be interpreted as a positive method result, hierarchy, uncertainty-calibration result, or novelty determination (§2.12/§7.15).

That is a coherent **diagnostic study**. It is not the broad “learnable selective memory for world models, robotics, and closed-loop control” paper implied by the present title, abstract, and result language. V8 is itself an adaptive response selected on the same development tasks, V9 is selected after opening V8, V10-J is an adaptive host repair whose ORBIT comparisons remain unrun, V11 is an adaptively designed successor stopped by its mandatory development screen, completed-negative V12 is another adaptive successor evaluated on the already opened four-task caches, and V13 is a frozen but still-unrun response to those opened failures (§7.10–7.15).
...
7. Treat completed V7–V12 strictly according to scope and V13 as unrun. V7–V9 have immutable negative labels. V10-J is an adaptive representation-validity audit with a `NO LAUNCH` decision; V11b is an excluded adaptive `SCREEN_NO_GO`, not a pilot, and its official grid is `OFFICIAL_NO_LAUNCH` at 0/240 and 0/400. V12 is complete excluded adaptive development at 28/28 and `SCREEN_NO_GO / NO_100E_LAUNCH`; V12b is a zero-optimizer-step train-only `STOP` audit, not a new model result. V13 is `READY_NOT_RUN` at 0/36 and 0/72; smoke telemetry is not evidence. A confirmation successor must use new tasks/trajectories, freeze the host before opening them, and add separately frozen executed-return evaluation plus tuned recurrent/SSM/Transformer baselines.

**Recommendation.** The V3–V9 prospective studies all return negative pilot/final labels. V10-J fixes the end-to-end collapse problem but fails its prerequisite state-quality/convergence audit. V11b and V12 are complete and negative; the correct closeouts remain V11 `SCREEN_NO_GO / OFFICIAL_NO_LAUNCH` and V12 `SCREEN_NO_GO / NO_100E_LAUNCH`. V13 is only `READY_NOT_RUN`: 0/36 screen, 0/72 continuation, no W&B evidence. Do **not** submit the current version to the ICLR main track, describe V10–V12 as positive memory results, or cite V13 smoke as a result. Retain the narrower workshop/diagnostic framing unless a separately frozen successor on new tasks clears representation, convergence, matched baselines, mechanism controls, and executed-return evaluation before any official launch.

## References

Gu & Dao. *Mamba: Linear-Time Sequence Modeling with Selective State Spaces.* 2023 (arXiv:2312.00752). · Ma et al. *Mega: Moving Average Equipped Gated Attention.* 2022 (arXiv:2209.10655). · Qin et al. *HGRN2: Gated Linear RNNs with State Expansion.* 2024 (arXiv:2404.07904). · Sun et al. *Retentive Network (RetNet).* 2023 (arXiv:2307.08621). · Ho & Kalman. *Effective Construction of Linear State-Variable Models from Input/Output Functions.* 1966. · Juang & Pappa. *An Eigensystem Realization Algorithm for Modal Parameter Identification and Model Reduction.* 1985. · Proctor, Brunton & Kutz. *Dynamic Mode Decomposition with Control.* 2016. · Littman, Sutton & Singh. *Predictive Representations of State.* 2001. · Becker et al. *Recurrent Kalman Networks: Factorized Inference in High-Dimensional Deep Feature Spaces.* ICML 2019 (arXiv:1905.07357). · Hefny et al. *Recurrent Predictive State Policy Networks.* ICML 2018. · Wisdom et al. *Full-Capacity Unitary Recurrent Neural Networks.* NeurIPS 2016 (arXiv:1611.00035). · Keurti et al. *Homomorphism Autoencoder.* 2022. · Ghaemi et al. *seq-JEPA.* NeurIPS 2025. · Lillemark et al. *Flow Equivariant World Modeling for Partially Observed Dynamic Environments.* 2025. · Behrouz et al. *Titans: Learning to Memorize at Test Time.* 2025 (arXiv:2501.00663). · *Instance-Conditional Timescales of Decay for Non-Stationary Learning.* (arXiv:2212.05908). · Yang et al. *Gated Linear Attention.* 2023 (arXiv:2312.06635). · Companion paper: `docs/ICLR.md` (§5.4 one global learned scalar decay does not self-tune in this setup; §5.11 fixed K-bank beats the tested learned memories under a matched budget).
